...
class SphereRenderer:
    ...
    def __init__(self):
        ...
        self.screen_camera_target = np.array([1.0, 0.0, 1.0])  # Virtual camera target (toward red cube)
    ...
    def get_cube_face_color(self, normal):
        ...
        if best_dot < 0.99:  # Show debug for any non-perfect match
            face_names = ["Front(+Z)", "Back(-Z)", "Top(+Y)", "Bottom(-Y)", "Right(+X)", "Left(-X)"]
    ...
    def trace_ray(self, ray_origin, ray_direction, depth=0):
        ...
        if hasattr(self, '_canvas_ref') and self._canvas_ref and hasattr(self._canvas_ref, 'red_cube_position'):
        ...
        if hasattr(self, '_canvas_ref') and self._canvas_ref and hasattr(self._canvas_ref, 'red_cube_size'):
            cube_size = self._canvas_ref.red_cube_size
            red_cube_size = np.array([cube_size, cube_size, cube_size])
        else:
            red_cube_size = np.array([0.3, 0.3, 0.3])  # Fallback to match simple mode default
        ...
        hit, t, hit_point, normal = self.ray_rotated_box_intersection(ray_origin, ray_direction, red_cube_pos, red_cube_size, cube_rotation)
        ...
        if depth == 0:  # Only print for primary rays to avoid spam
            ...
            ideal_ray = red_cube_pos / np.linalg.norm(red_cube_pos)  # (0.707, 0, 0.707)
            ...
            if dot_to_ideal > 0.95:  # Close to ideal ray (lowered threshold)
                ...
                print(f"  - Cube bounds: [{red_cube_pos - red_cube_size}] to [{red_cube_pos + red_cube_size}]")
        ...
        if depth == 0 and closest_material == "red_cube" and np.random.random() < 0.01:  # Only 1% of hits
    ...
    def render_screen_texture(self):
            ...
            for x in range(0, render_width, step):
                ...
                if self.screen_render_mode == "ray_marching":
                    ...
                    if np.any(color > 0.4) and color[0] > color[1] and color[0] > color[2]:  # Red-ish color (cube hit)
                        red_cube_hits += 1
                    elif np.allclose(color, [0.1, 0.1, 0.1], atol=0.05):  # Background color
                        background_hits += 1
        ...
        print(f"  - Other hits: {total_rays - red_cube_hits - background_hits}")
        print(f"  - Cube hit percentage: {(red_cube_hits/total_rays*100):.2f}%")
    ...
    def _render_truncated_cuboid(self, direction, width_axis, height_axis):
        ...
        smaller_center = near_plane_pos + direction * (remaining_length / 2.0)
    ...
    def _render_sphere_cuboid_intersection(self):
        ...
        for i in range(resolution):
            for j in range(resolution):
                ...
                length_coords = [cuboid_length * 0.1, cuboid_length * 0.9]  # Stay within cuboid bounds
                width_coords = [half_width * (2 * u1 - 1), half_width * (2 * u2 - 1)]  # -half_width to +half_width
    ...
    def load_scene_from_dict(self, scene_data: dict):
        ...
        try:
            ...
            if "sphere" in scene_data:
                ...
                self.position = np.array(s.get("position", [0.0, 0.0, 0.0]))
                self.rotation = np.array(s.get("rotation", [0.0, 0.0, 0.0]))
                self.scale = np.array(s.get("scale", [1.0, 1.0, 1.0]))
                self.sphere_color = np.array(s.get("color", [0.2, 0.5, 1.0, 0.7]))
                ...
                self.wireframe_resolution = s.get("wireframe_resolution", 16)
            
            # Vector properties
            if "vector" in scene_data:
                v = scene_data["vector"]
                self.vector_enabled = v.get("enabled", True)
                self.vector_direction = np.array(v.get("direction", [1.0, 0.0, 0.0]))
                self.vector_length = v.get("length", 2.0)
                self.vector_color = np.array(v.get("color", [1.0, 0.0, 0.0, 1.0]))
            ...
            if "cone" in scene_data:
                ...
                self.cone_color = np.array(c.get("color", [1.0, 1.0, 0.0, 0.3]))
            ...
            if "pyramid" in scene_data:
                ...
                self.pyramid_angle_horizontal = p.get("angle_horizontal", 25.0)
                self.pyramid_angle_vertical = p.get("angle_vertical", 20.0)
                self.pyramid_color = np.array(p.get("color", [0.0, 1.0, 1.0, 0.3]))
            ...
            if "cuboid" in scene_data:
                ...
                self.cuboid_color = np.array(cb.get("color", [1.0, 0.0, 1.0, 0.3]))
            ...
            if "screen" in scene_data:
                ...
                self.screen_position = np.array(sc.get("position", [0.0, 0.0, -3.0]))
                self.screen_rotation = np.array(sc.get("rotation", [0.0, 0.0, 0.0]))
                self.screen_width = sc.get("width", 2.0)
                self.screen_height = sc.get("height", 1.5)
                self.screen_resolution = tuple(sc.get("resolution", [128, 96]))
            ...
            if "sphere_intersection" in scene_data:
                ...
                self.sphere_intersection_color = np.array(si.get("color", [1.0, 0.5, 0.0, 0.8]))
            ...
            if "normal_rays" in scene_data:
                ...
                self.normal_rays_color = np.array(nr.get("color", [0.0, 1.0, 0.0, 1.0]))
            ...
            if "intersection_normals" in scene_data:
                ...
                self.intersection_normals_thickness = inr.get("thickness", 1.0)
            ...
            if "truncation_normals" in scene_data:
                ...
                self.truncation_normals_color = np.array(tn.get("color", [1.0, 0.0, 1.0, 1.0]))
                self.truncation_normals_thickness = tn.get("thickness", 1.0)
...
class Sphere3DCanvas(wx.glcanvas.GLCanvas):
    ...
    def __init__(self, parent):
        ...
        self.red_cube_position = np.array([2.0, 0.0, 2.0])  # Fixed position in world space
    ...
    def render_simple_sphere_view(self):
        ...
        angle_to_cube = math.degrees(math.acos(np.clip(dot_with_cube_dir, -1.0, 1.0)))
    ...
    def reset_selected_object(self):
        ...
        if self.selected_object in self.object_rotations:
            self.object_rotations[self.selected_object] = np.array([0.0, 0.0, 0.0])
    ...
    def set_screen_render_mode(self, mode):
        """Set the unified screen's rendering mode: 'simple' or 'raytracing'."""
    ...
    def update_sphere_view_vector(self):
        ...
        cube_direction = (self.red_cube_position - self.sphere.position) / np.linalg.norm(self.red_cube_position - self.sphere.position)
    ...
    def load_canvas_scene_from_dict(self, canvas_data: dict):
        ...
        try:
            ...
            if "camera" in canvas_data:
                ...
                self.camera_position = np.array(c.get("position", [0.0, 0.0, 0.0]))
                self.camera_projection = c.get("projection", "perspective")
            ...
            if "screen" in canvas_data:
                ...
                self.screen_projection = s.get("projection", "perspective")
                self.screen_position = np.array(s.get("position", [0.0, 0.0, -3.0]))
            ...
            if "cube" in canvas_data:
                cb = canvas_data["cube"]
                self.red_cube_position = np.array(cb.get("position", [2.0, 0.0, 2.0]))
...
class Sphere3DFrame(wx.Frame):
    ...
    def create_menu_bar(self):
        ...
        file_menu.Append(wx.ID_SAVEAS, "Save Scene &As...\tCtrl+Shift+S", "Save scene with a new name")
        file_menu.AppendSeparator()
        ...
        vector_menu.AppendCheckItem(wx.ID_ANY, "Show Vector Orientation", "Toggle vector orientation indicator")
        ...
        vector_orientation_menu.Append(wx.ID_ANY, "Set Roll Angle...", "Set vector roll orientation angle")
        ...
        cone_props_menu.AppendCheckItem(wx.ID_ANY, "Infinite Cone Length", "Make cone extend infinitely")
        ...
        cuboid_props_menu.Append(wx.ID_ANY, "Set Cuboid Transparency...", "Set cuboid transparency")
        ...
        near_plane_menu.AppendCheckItem(wx.ID_ANY, "Enable Near Plane", "Toggle near plane cutting of shapes")
        near_plane_menu.Append(wx.ID_ANY, "Set Near Plane Distance...", "Set distance of cutting plane from sphere center")
        ...
        camera_movement_menu.Append(wx.ID_ANY, "Set Camera Position...", "Set camera position (x,y,z)")
        camera_movement_menu.Append(wx.ID_ANY, "Set Camera Distance...", "Set camera distance from origin")
        ...
        camera_movement_menu.AppendSeparator()
        camera_movement_menu.Append(wx.ID_ANY, "Increase Movement Speed", "Make camera move faster")
        camera_movement_menu.Append(wx.ID_ANY, "Decrease Movement Speed", "Make camera move slower")
        ...
        sphere_movement_menu.Append(wx.ID_ANY, "Set Sphere Position...", "Set sphere position (affects sphere + vector + shapes only)")
        sphere_movement_menu.AppendSeparator()
        sphere_movement_menu.Append(wx.ID_ANY, "Move Sphere Forward", "Move sphere forward (cube and screen stay in place)")
        sphere_movement_menu.Append(wx.ID_ANY, "Move Sphere Backward", "Move sphere backward")
        ...
        view_menu.AppendSeparator()
        view_menu.AppendCheckItem(wx.ID_ANY, "Show Intersection Normals", "Show normal rays on intersection surfaces")
        ...
        view_menu.AppendSeparator()
        view_menu.AppendCheckItem(wx.ID_ANY, "Show Truncation Normals", "Show normal rays on truncation surfaces")
        ...
        # Main screen toggle
        screen_menu.AppendCheckItem(wx.ID_ANY, "Show Ray Tracing Screen", "Show 2D screen with ray-traced image")
        ...
        render_mode_menu.AppendRadioItem(wx.ID_ANY, "PBR Rendering", "Physically Based Rendering")
        ...
        self.cube_selection_item = selection_menu.AppendRadioItem(wx.ID_ANY, "Red Cube", "Select red cube for manipulation")
        ...
        rotation_submenu.Append(wx.ID_ANY, "Set Selected Rotation...", "Set selected object rotation (pitch, yaw, roll)")
        ...
        rotation_submenu.Append(wx.ID_ANY, "Rotate -15° Z", "Rotate selected object -15° around Z axis")
        rotation_submenu.AppendSeparator()
        rotation_submenu.Append(wx.ID_ANY, "Reset Selected Rotation", "Reset selected object rotation to zero")
        manipulation_menu.AppendSubMenu(rotation_submenu, "Rotate Selected Object")
        
        # Position controls
        position_submenu = wx.Menu()
        position_submenu.Append(wx.ID_ANY, "Set Selected Position...", "Set selected object position (x, y, z)")
        ...
        position_submenu.AppendSeparator()
        position_submenu.Append(wx.ID_ANY, "Reset Selected Position", "Reset selected object to default position")
        ...
        camera_menu.Append(wx.ID_ANY, "Set Pyramid Horizontal Angle...", "Set pyramid horizontal angle (affects 2D camera horizontal FOV)")
        camera_menu.Append(wx.ID_ANY, "Set Pyramid Vertical Angle...", "Set pyramid vertical angle (affects 2D camera vertical FOV)")
        ...
        self.raytracing_render_item = screen_render_menu.AppendRadioItem(wx.ID_ANY, "Ray Tracing Rendering", "Advanced ray tracing rendering")
    ...
    def show_object_properties(self):
        ...
        if selected == "none":
            wx.MessageBox("No object selected", "Object Properties", wx.OK | wx.ICON_INFORMATION)
            return
    ...
    def set_vector_roll(self):
        ...
        if dialog.ShowModal() == wx.ID_OK:
            ...
            self.canvas.Refresh()
            wx.MessageBox(f"Vector roll set to: {dialog.GetValue()}°", "Roll Updated", wx.OK | wx.ICON_INFORMATION)
            dialog.Destroy()
    ...
    def set_camera_cone_angle(self):
        ...
        dialog = wx.NumberEntryDialog(self, "Enter cone angle (degrees):", "Angle:", "Cone Angle", 
    ...
    def set_camera_cuboid_width(self):
        ...
        dialog = wx.NumberEntryDialog(self, "Enter cuboid width:", "Width:", "Cuboid Width", 
    ...
    def set_camera_cuboid_height(self):
        ...
        dialog = wx.NumberEntryDialog(self, "Enter cuboid height:", "Height:", "Cuboid Height", 
...
• Multicolored cube and 2D screen stay in fixed world positions!"""
    ...
    def reset_selected_object_rotation(self):
        ...
        if selected in self.canvas.object_rotations:
            self.canvas.object_rotations[selected] = np.array([0.0, 0.0, 0.0])
            self.canvas.Refresh()
            wx.MessageBox(f"{selected.title()} rotation reset to zero", "Rotation Reset", wx.OK | wx.ICON_INFORMATION)
                ...
                try:
                    ...
                    if len(values) == 3:
                        if selected == "sphere":
                            ...
                            self.canvas.red_cube_position = np.array(values)
                        elif selected == "screen":
    ...
    def reset_selected_object_position(self):
        ...
        if selected == "sphere":
            self.sphere.set_position(0.0, 0.0, 0.0)
            wx.MessageBox("Sphere position reset to origin (0,0,0)", "Position Reset", wx.OK | wx.ICON_INFORMATION)
        elif selected == "cube":
            self.canvas.red_cube_position = np.array([2.0, 0.0, 2.0])
            wx.MessageBox("Cube position reset to default (2,0,2)", "Position Reset", wx.OK | wx.ICON_INFORMATION)
        elif selected == "screen":
            self.canvas.screen_position = np.array([-3.0, 0.0, 0.0])
            wx.MessageBox("Screen position reset to default (-3,0,0)", "Position Reset", wx.OK | wx.ICON_INFORMATION)
    ...
    def center_camera_on_cube(self):
        ...
        self.canvas.Refresh()
        wx.MessageBox(f"Camera centered on cube at position {cube_pos}", "Camera Positioned", wx.OK | wx.ICON_INFORMATION)
    ...
    def set_cone_transparency(self):
        ...
        dialog = wx.NumberEntryDialog(self, "Enter transparency (0-100):", "Transparency:", "Cone Transparency", current_alpha, 0, 100)
        if dialog.ShowModal() == wx.ID_OK:
            alpha = dialog.GetValue() / 100.0
    ...
    def set_pyramid_transparency(self):
        ...
        dialog = wx.NumberEntryDialog(self, "Enter transparency (0-100):", "Transparency:", "Pyramid Transparency", current_alpha, 0, 100)
        if dialog.ShowModal() == wx.ID_OK:
            alpha = dialog.GetValue() / 100.0
    ...
    def set_cuboid_transparency(self):
        ...
        dialog = wx.NumberEntryDialog(self, "Enter transparency (0-100):", "Transparency:", "Cuboid Transparency", current_alpha, 0, 100)
        if dialog.ShowModal() == wx.ID_OK:
            alpha = dialog.GetValue() / 100.0
    ...
    def set_normal_ray_length(self):
        ...
        dialog = wx.NumberEntryDialog(self, "Enter ray length (relative to sphere radius):", "Length:", "Normal Ray Length", int(current_length * 10), 1, 20)
        if dialog.ShowModal() == wx.ID_OK:
    ...
    def set_normal_ray_thickness(self):
        ...
        dialog = wx.NumberEntryDialog(self, "Enter ray thickness (1-5):", "Thickness:", "Normal Ray Thickness", int(current_thickness), 1, 5)
        if dialog.ShowModal() == wx.ID_OK:
    ...
    def set_intersection_ray_length(self):
        ...
        if dialog.ShowModal() == wx.ID_OK:
            self.sphere.set_intersection_normals_length(dialog.GetValue() / 10.0)
            self.canvas.Refresh()
            dialog.Destroy()
    ...
    def set_truncation_ray_length(self):
        ...
        if dialog.ShowModal() == wx.ID_OK:
            self.sphere.set_truncation_normals_length(dialog.GetValue() / 10.0)
            self.canvas.Refresh()
            dialog.Destroy()
    ...
    def set_screen_update_rate(self):
        ...
        if dialog.ShowModal() == wx.ID_OK:
            ...
            wx.MessageBox(f"Screen update rate set to {rate:.1f} seconds", "Rate Updated", wx.OK | wx.ICON_INFORMATION)
        dialog.Destroy()
    ...
    def set_screen_samples(self):
        ...
        if dialog.ShowModal() == wx.ID_OK:
            ...
            wx.MessageBox(f"Screen samples set to {dialog.GetValue()}", "Samples Updated", wx.OK | wx.ICON_INFORMATION)
        dialog.Destroy()
    ...
    def _save_scene_to_file(self, filepath: str):
        ...
        try:
            ...
            wx.MessageBox(f"Scene saved successfully to:\n{filepath}", "Scene Saved", wx.OK | wx.ICON_INFORMATION)
