...
class GraphCanvas(wx.Panel, GraphCanvasPropertyNotifierMixin):
    ...
    def __init__(self, parent, graph: m_graph.Graph, undo_redo_manager=None):
        ...
        self.checker_color2 = wx.Colour(255, 255, 255)  # White (will be set to background color)
        ...
        self.drag_original_positions = {}  # node_id -> (original_x, original_y) for undo
        ...
        self.temp_path_points = []  # For storing freeform path points during edge creation
        ...
        self.dragging_selection = []  # Nodes/edges being dragged into container
        ...
        self.grid_color = (100, 100, 100)  # Darker grid color for better visibility
        
        print(f"DEBUG: Canvas initialized with grid_style='{self.grid_style}', spacing={self.grid_spacing}")
        print(f"DEBUG: Grid color: {self.grid_color}, Background: {self.background_color}")
        ...
        # Set up canvas
        self.SetBackgroundColour(wx.Colour(*self.background_color))  # Use RGB tuple
        ...
        print(f"DEBUG: Zoom controls: Trackpad scroll, +/- keys, Arrow keys for pan")
    ...
    def on_mousewheel(self, event):
        ...
        if self.zoom_event_in_progress:
            print(f"DEBUG: Skipping mousewheel #{self.zoom_event_counter} - zoom event already in progress")
            return
        ...
        try:
            ...
            delta = event.GetWheelDelta() or 120  # Fallback to common notch value
            ...
            print(f"DEBUG: frac_steps: {frac_steps:.3f}, exponent: {exponent:.3f}, base_factor: {base_factor:.3f}")
            ...
            cur_sign = 1 if exponent > self._zoom_direction_epsilon else (-1 if exponent < -self._zoom_direction_epsilon else 0)
            if self.zoom_active:
                if self.zoom_gesture_sign == 0 and cur_sign != 0:
                    self.zoom_gesture_sign = cur_sign
                    print(f"DEBUG: Zoom gesture start sign={self.zoom_gesture_sign}")
                elif cur_sign != 0 and cur_sign != self.zoom_gesture_sign:
                    # Opposite tiny step? suppress it
                    if abs(exponent) < 0.08:
                        print("DEBUG: Suppressing opposite tiny wheel step to prevent direction flip")
                        zoom_factor = 1.0
                        cur_sign = 0
                    else:
                        # Significant reversal: accept and update sign
                        self.zoom_gesture_sign = cur_sign
                        print(f"DEBUG: Zoom gesture sign updated to {self.zoom_gesture_sign}")
            print(f"DEBUG: zoom factor: pow({base_factor:.3f}, {exponent:.3f}) = {zoom_factor:.6f}")
            ...
            try:
                ...
                alt = event.AltDown()
                print(f"DEBUG: 🖱️ Mouse state L={left_down} M={middle_down} R={right_down} | Ctrl={ctrl} Shift={shift} Alt={alt}")
            except Exception:
                pass
            ...
            print(f"DEBUG: mouse_pos from event: ({mouse_pos.x}, {mouse_pos.y})")
            print(f"DEBUG: Updated current_mouse_pos: ({self.current_mouse_pos.x}, {self.current_mouse_pos.y})")
            ...
            if self._zoom_anchor_screen is None:
                ...
                self._zoom_anchor_world = self.screen_to_world(mouse_pos.x, mouse_pos.y)
                print(f"DEBUG: 📌 Captured zoom anchor: screen={self._zoom_anchor_screen}, world=({self._zoom_anchor_world[0]:.3f}, {self._zoom_anchor_world[1]:.3f})")
            ...
            if not self._zoom_apply_scheduled:
                ...
                wx.CallLater(self._zoom_apply_delay_ms, self._apply_accumulated_zoom)
            ...
            print(f"DEBUG: ===== MOUSE WHEEL EVENT #{self.zoom_event_counter} END =====")
            print()
    ...
    def on_magnify(self, event):
        ...
        try:
            ...
            print(f"DEBUG: Magnify -> mode={mode}, computed zoom_factor={zoom_factor:.6f}")

            # Debounce zoom direction within a gesture to avoid in/out flicker
            cur_sign = 1 if zoom_factor > 1.0 + self._zoom_direction_epsilon else (-1 if zoom_factor < 1.0 - self._zoom_direction_epsilon else 0)
            if self.zoom_active:
                if self.zoom_gesture_sign == 0 and cur_sign != 0:
                    self.zoom_gesture_sign = cur_sign
                    print(f"DEBUG: Magnify gesture start sign={self.zoom_gesture_sign}")
                elif cur_sign != 0 and cur_sign != self.zoom_gesture_sign:
                    ...
                    if mag_strength < 0.02:
                        print("DEBUG: Suppressing opposite tiny magnify step to prevent direction flip")
                        zoom_factor = 1.0
                        cur_sign = 0
                    else:
                        self.zoom_gesture_sign = cur_sign
                        print(f"DEBUG: Magnify gesture sign updated to {self.zoom_gesture_sign}")
            ...
            mouse_pos = event.GetPosition()
            self.current_mouse_pos = mouse_pos
            print(f"DEBUG: 🖱️ Magnify zoom at mouse position ({self.current_mouse_pos.x}, {self.current_mouse_pos.y}) with factor {magnification:.3f}")
    ...
    def on_gesture_zoom(self, event):
        ...
        if self.zoom_event_in_progress:
            print("DEBUG: Skipping gesture zoom - zoom event already in progress")
            event.Skip()
            return
        ...
        if hasattr(event, 'GetZoomFactor'):
            ...
            try:
                ...
                print(f"DEBUG: 🖱️ Gesture zoom at current_mouse_pos ({self.current_mouse_pos.x}, {self.current_mouse_pos.y}) with factor {zoom_factor:.3f}")
    ...
    def on_gesture_pan(self, event):
        ...
        if hasattr(event, 'GetDelta'):
            ...
            if getattr(self, 'zoom_event_in_progress', False) or getattr(self, 'zoom_active', False) or getattr(self, 'pan_suppressed', False):
    ...
    def on_key_down(self, event):
        ...
        if (key_code == wx.WXK_LEFT or key_code == ord('A')) and not self.pan_suppressed:
            self.pan_x += pan_speed
            self.Refresh()
        elif (key_code == wx.WXK_RIGHT or key_code == ord('D')) and not self.pan_suppressed:
            self.pan_x -= pan_speed
            self.Refresh()
        elif (key_code == wx.WXK_UP or key_code == ord('W')) and not self.pan_suppressed:
            self.pan_y += pan_speed
            self.Refresh()
        elif (key_code == wx.WXK_DOWN or key_code == ord('S')) and not self.pan_suppressed:
            self.pan_y -= pan_speed
            self.Refresh()
        # Zoom with + and - keys
            ...
            if self.center_zoom_enabled:
                ...
                self.zoom_at_point(1.0 + (0.2 * self.zoom_sensitivity), self.current_mouse_pos)
                print(f"DEBUG: 🖱️ Keyboard zoom in at mouse position ({self.current_mouse_pos.x}, {self.current_mouse_pos.y})")
        elif key_code == ord('-'):
            if self.center_zoom_enabled:
                # Use center-based zooming
                ...
                # Use cursor-based zooming (original behavior)
                self.zoom_at_point(1.0 - (0.2 * self.zoom_sensitivity), self.current_mouse_pos)
                print(f"DEBUG: 🖱️ Keyboard zoom out at mouse position ({self.current_mouse_pos.x}, {self.current_mouse_pos.y})")
    ...
    def set_move_sensitivity(self, x_sensitivity: float, y_sensitivity: float, inverted: bool):
        ...
        print(f"DEBUG: Move sensitivity set to X:{x_sensitivity}, Y:{y_sensitivity}, Inverted:{inverted}")
    ...
    def _apply_accumulated_zoom(self):
        ...
        try:
            if self._zoom_anchor_screen is None or abs(self._zoom_accum_ln) < 1e-6:
            ...
            zoom_factor = max(0.01, min(100.0, zoom_factor))
            anchor_point = wx.Point(self._zoom_anchor_screen[0], self._zoom_anchor_screen[1])
            print(f"DEBUG: 🚀 Applying accumulated zoom: ln={self._zoom_accum_ln:.6f}, factor={zoom_factor:.6f} at anchor=({anchor_point.x},{anchor_point.y})")
    ...
    def zoom_at_point(self, zoom_factor: float, point: wx.Point):
        ...
        if not point:
            ...
            print(f"DEBUG: No point provided, using center: ({point.x}, {point.y})")
        ...
        print(f"DEBUG: 1. screen_to_world({point.x}, {point.y}) = ({world_point[0]:.3f}, {world_point[1]:.3f})")
        
        # Verify the reverse transformation works before zoom
        check_screen_x, check_screen_y = self.world_to_screen(world_point[0], world_point[1])
        pre_zoom_error_x = abs(point.x - check_screen_x)
        pre_zoom_error_y = abs(point.y - check_screen_y)
        print(f"DEBUG: 2. world_to_screen({world_point[0]:.3f}, {world_point[1]:.3f}) = ({check_screen_x}, {check_screen_y})")
        print(f"DEBUG: 3. Pre-zoom round-trip error: ({pre_zoom_error_x}, {pre_zoom_error_y})")
        
        if pre_zoom_error_x > 1 or pre_zoom_error_y > 1:
            print(f"DEBUG: ⚠️  WARNING: Pre-zoom coordinate transformation has errors!")
        else:
        ...
        if abs(zoom_factor - 1.0) < 0.001:
            print("DEBUG: Zoom factor is 1.0 - no zoom change requested, returning early")
            return
        ...
        print(f"DEBUG: Mouse world position (must stay fixed): ({world_x:.3f}, {world_y:.3f})")
        ...
        print(f"DEBUG: 🔁 Forward-check: world_to_screen(world_point) = ({precheck_x:.3f}, {precheck_y:.3f}), anchor=({point.x}, {point.y}), err=({anchor_err_x:.3f}, {anchor_err_y:.3f})")
        ...
        print(f"DEBUG: pan set: ({old_pan_x:.3f}, {old_pan_y:.3f}) -> ({self.pan_x:.3f}, {self.pan_y:.3f})")
        ...
        print(f"DEBUG: 🔎 world_to_screen(post): ({final_screen_x}, {final_screen_y}) vs anchor=({point.x}, {point.y}) -> err=({final_err_x}, {final_err_y})")
        ...
        print(f"DEBUG: World position after zoom: ({world_after[0]:.3f}, {world_after[1]:.3f})")
        print(f"DEBUG: World position error: ({world_error_x:.3f}, {world_error_y:.3f})")
        ...
        if world_error_x > 0.1 or world_error_y > 0.1:
            ...
            print(f"DEBUG: 🔍 World after: ({world_after[0]:.3f}, {world_after[1]:.3f})")
            print(f"DEBUG: 🔍 Error: ({world_error_x:.3f}, {world_error_y:.3f})")
        else:
        ...
        if hasattr(self, 'zoom_center_locked') and self.zoom_center_locked:
            ...
            print(f"DEBUG: Zoom center world pos: {self.zoom_center_world_pos}")
            print(f"DEBUG: Zoom center screen pos: {self.zoom_center_screen_pos}")
            ...
            if self.zoom_center_world_pos:
                ...
                print(f"DEBUG: Crosshair world position: ({crosshair_world_x:.3f}, {crosshair_world_y:.3f})")
                print(f"DEBUG: Fixed point world position: ({world_x:.3f}, {world_y:.3f})")
                print(f"DEBUG: Crosshair vs fixed point error: ({crosshair_error_x:.3f}, {crosshair_error_y:.3f})")
                
                if crosshair_error_x > 0.1 or crosshair_error_y > 0.1:
                    print(f"DEBUG: ⚠️  WARNING: Crosshair world position doesn't match fixed point!")
                else:
                    print(f"DEBUG: ✅ Crosshair world position matches fixed point!")
            ...
            if self.zoom_center_screen_pos:
                ...
                print(f"DEBUG: Crosshair screen position: ({crosshair_screen_x}, {crosshair_screen_y})")
                print(f"DEBUG: Zoom point screen position: ({point.x}, {point.y})")
                print(f"DEBUG: Crosshair vs zoom point screen error: ({screen_error_x}, {screen_error_y})")
                
                if screen_error_x > 1 or screen_error_y > 1:
                    print(f"DEBUG: ⚠️  WARNING: Crosshair screen position doesn't match zoom point!")
                else:
                    print(f"DEBUG: ✅ Crosshair screen position matches zoom point!")
        
        # Update canvas information display
        if hasattr(self, 'main_window') and hasattr(self.main_window, 'update_canvas_info'):
        ...
        print(f"DEBUG: World point (should be unchanged): ({world_point[0]:.3f}, {world_point[1]:.3f})")
        print(f"DEBUG: Old zoom: {old_zoom:.3f}, New zoom: {self.zoom:.3f}")
        print(f"DEBUG: Old pan: ({old_pan_x:.3f}, {old_pan_y:.3f}), New pan: ({self.pan_x:.3f}, {self.pan_y:.3f})")
        
        # The world point should now map back to the original screen position
        final_screen_x, final_screen_y = self.world_to_screen(world_point[0], world_point[1])
        final_error_x = abs(point.x - final_screen_x)
        final_error_y = abs(point.y - final_screen_y)
        
        print(f"DEBUG: world_to_screen({world_point[0]:.3f}, {world_point[1]:.3f}) = ({final_screen_x}, {final_screen_y})")
        print(f"DEBUG: Final transformation error: ({final_error_x}, {final_error_y})")
        ...
        print(f"DEBUG: Error vs current_mouse_pos ({self.current_mouse_pos.x}, {self.current_mouse_pos.y}): ({mouse_error_x}, {mouse_error_y})")
        
        if final_error_x > 1 or final_error_y > 1:
            print(f"DEBUG: ⚠️  INVARIANT VIOLATION: World point doesn't map back to original screen position!")
            print(f"DEBUG: This is the source of the boomerang effect!")
        else:
            print(f"DEBUG: ✅ INVARIANT SATISFIED: Zoom-at-point working correctly")
    ...
    def reset_view(self):
        ...
        print(f"DEBUG: View reset to saved defaults: zoom={self.zoom}, pan=({self.pan_x}, {self.pan_y}), rotation={self.world_rotation}°")
        self.Refresh()
    ...
    def force_grid_visible(self):
        ...
        print(f"DEBUG: Grid forced - style: {self.grid_style}, spacing: {self.grid_spacing}")
        print(f"DEBUG: Colors - grid: {self.grid_color}, background: {self.background_color}")
        self.Refresh()
    ...
    def create_test_background(self):
        ...
        test_image.SetRGB(wx.Rect(50, 50, 100, 100), 200, 100, 100)  # Light red square
    ...
    def set_custom_origin(self):
        ...
        print(f"DEBUG: Custom origin set to pan=({self.custom_origin_x}, {self.custom_origin_y})")
    ...
    def set_custom_default_rotation(self):
        ...
        print(f"DEBUG: Custom default rotation set to {self.custom_default_rotation}°")
    ...
    def zoom_to_fit(self, padding=50):
        ...
        dynamic_padding = max(content_width, content_height) * 0.1  # 10% of content size
        ...
        print(f"DEBUG: Zoom to fit - Content bounds: ({min_x:.1f}, {min_y:.1f}) to ({max_x:.1f}, {max_y:.1f})")
        print(f"DEBUG: Zoom to fit - Zoom: {new_zoom:.2f}, Pan: ({new_pan_x:.1f}, {new_pan_y:.1f})")
    ...
    def _get_edge_points(self, edge, source_node, target_node) -> List[Tuple[float, float]]:
    ...
    def transform_direction_world_to_screen(self, dx: float, dy: float) -> Tuple[float, float]:
    ...
    def _screen_delta_to_pan_delta(self, dx: float, dy: float) -> Tuple[float, float]:
    ...
    def screen_to_world_delta(self, screen_dx: int, screen_dy: int) -> Tuple[float, float]:
        ...
        if self.world_rotation != 0.0:
            rotation_rad = -math.radians(self.world_rotation)  # Negative for inverse
            cos_angle = math.cos(rotation_rad)
            sin_angle = math.sin(rotation_rad)
    ...
    def handle_edge_scrolling_and_cursor_mirror(self, current_pos, is_world_drag=False):
        ...
        if self.world_rotation != 0.0 and not is_world_drag:
            ...
            print(f"DEBUG: 🎮 Cursor mirroring: actual({current_pos.x}, {current_pos.y}) -> effective({effective_pos.x}, {effective_pos.y})")
        ...
        if current_pos.x < self.edge_scroll_zone:
            edge_scroll_x = -(self.edge_scroll_zone - current_pos.x) * self.edge_scroll_speed
        # Right edge  
        elif current_pos.x > size.width - self.edge_scroll_zone:
            edge_scroll_x = (current_pos.x - (size.width - self.edge_scroll_zone)) * self.edge_scroll_speed
        
        # Top edge
        if current_pos.y < self.edge_scroll_zone:
            edge_scroll_y = -(self.edge_scroll_zone - current_pos.y) * self.edge_scroll_speed
        # Bottom edge
        elif current_pos.y > size.height - self.edge_scroll_zone:
            edge_scroll_y = (current_pos.y - (size.height - self.edge_scroll_zone)) * self.edge_scroll_speed
        
        # Apply edge scrolling
        if edge_scroll_x != 0 or edge_scroll_y != 0:
            print(f"DEBUG: 📏 Edge scrolling: ({edge_scroll_x:.1f}, {edge_scroll_y:.1f})")
    ...
    def zoom_to_world_center(self, zoom_in=True):
        ...
        print(f"DEBUG: 🔍 Center zoom: {old_zoom:.2f} -> {new_zoom:.2f}, {'in' if zoom_in else 'out'}")
        self.Refresh()
    ...
    def zoom_to_world_center_magnify(self, magnification):
        ...
        old_zoom = self.zoom
        new_zoom = max(self.min_zoom, min(self.max_zoom, self.zoom * magnification))
        self.zoom = new_zoom
    ...
    def snap_to_grid(self, x: float, y: float, node_width: float = 50.0, node_height: float = 30.0) -> Tuple[float, float]:
        ...
        print(f"DEBUG: 🔗 GRID SNAP: node_center ({x:.1f}, {y:.1f}) -> ({snapped_x:.1f}, {snapped_y:.1f})")
        print(f"DEBUG: 🔗 GRID SNAP: anchor '{self.snap_anchor}' at ({anchor_x:.1f}, {anchor_y:.1f}) -> snapped to ({snapped_anchor_x:.1f}, {snapped_anchor_y:.1f})")
        print(f"DEBUG: 🔗 GRID SNAP: grid intersections: ...,-40,-20,0,20,40,60,... (spacing={grid_size})")
    ...
    def point_near_curve(self, px, py, edge, source_node, target_node, tolerance):
        ...
        print(f"DEBUG: 🎯 Checking if point ({px:.1f}, {py:.1f}) is near edge {edge.id} (type: {rendering_type}, tolerance: {tolerance:.1f})")
        ...
        for i in range(samples + 1):
            t = i / samples
            curve_point = self.calculate_position_on_curve(edge, source_node, target_node, t)
            
            if curve_point:
                if prev_point:
                    # Check distance to line segment between prev_point and curve_point
                    distance = self._point_to_line_segment_distance(px, py, prev_point, curve_point)
                    min_distance = min(min_distance, distance)
                    if distance <= tolerance:
                        print(f"DEBUG: 🎯 Point is within tolerance! Distance: {distance:.1f} <= {tolerance:.1f}")
                        return True
                prev_point = curve_point
        
        print(f"DEBUG: 🎯 Point not near edge {edge.id}. Min distance: {min_distance:.1f}")
        return False
    ...
    def calculate_anchor_position(self, node, mouse_world_pos, anchor_mode):
        ...
        if anchor_mode == "free":
            ...
            if effective_rotation != 0.0:
                ...
                cos_rot = math.cos(math.radians(-effective_rotation))  # Inverse rotation
                sin_rot = math.sin(math.radians(-effective_rotation))
            ...
            return rotate_point_around_node(node.x + local_dx, node.y + local_dy)
    ...
    def update_custom_endpoints_for_moved_nodes(self, old_positions):
        ...
        for edge in self.graph.get_all_edges():
            if not hasattr(edge, 'custom_endpoints') or not edge.custom_endpoints:
                continue
            ...
            if edge.source_id in old_positions and 'source' in edge.custom_endpoints:
                ...
                new_endpoint = (new_node_pos[0] + offset_x, new_node_pos[1] + offset_y)
                edge.custom_endpoints['source'] = new_endpoint
                
                print(f"DEBUG: 🔗 Updated source endpoint for edge {edge.id}: {old_endpoint} -> {new_endpoint}")
            ...
            if edge.target_id in old_positions and 'target' in edge.custom_endpoints:
                ...
                print(f"DEBUG: 🔗 Updated target endpoint for edge {edge.id}: {old_endpoint} -> {new_endpoint}")
    ...
    def update_control_points_for_moved_nodes(self, old_positions):
        ...
        for edge in self.graph.get_all_edges():
            ...
            if is_self_loop and edge.source_id in old_positions:
                ...
                new_control_points = []
                for cp in edge.control_points:
                    if len(cp) >= 2:  # Ensure we have at least x, y coordinates
                ...
                print(f"DEBUG: 🎛️ Updated {len(new_control_points)} control points for self-loop edge {edge.id}")
                
            # For edges between different nodes
            elif not is_self_loop and (edge.source_id in old_positions or edge.target_id in old_positions):
                ...
                if source_moved and target_moved:
                    ...
                    for cp in edge.control_points:
                        if len(cp) >= 2:
                            ...
                            if abs(old_edge_dx) > 0.1 or abs(old_edge_dy) > 0.1:  # Avoid division by zero
                                ...
                                if abs(old_edge_dx) > abs(old_edge_dy):
                                    t = (cp[0] - old_source_pos[0]) / old_edge_dx if old_edge_dx != 0 else 0.5
                                else:
                                    t = (cp[1] - old_source_pos[1]) / old_edge_dy if old_edge_dy != 0 else 0.5
                    ...
                    edge.control_points = new_control_points
                    print(f"DEBUG: 🎛️ Updated {len(new_control_points)} control points for edge {edge.id} (both nodes moved)")
                    ...
                    new_moved_pos = (source_node.x, source_node.y) if source_moved else (target_node.x, target_node.y)
                    ...
                    for cp in edge.control_points:
                        if len(cp) >= 2:
                            new_cp = (cp[0] + dx * fraction, cp[1] + dy * fraction)
                            if len(cp) > 2:  # Preserve z coordinate if present
                                new_cp = new_cp + (cp[2],)
                            new_control_points.append(new_cp)
                    
                    edge.control_points = new_control_points
                    print(f"DEBUG: 🎛️ Updated {len(new_control_points)} control points for edge {edge.id} ({'source' if source_moved else 'target'} moved)")
    ...
    def graham_scan(self, points):
        ...
        bottom_idx = 0
        for i in range(1, n):
            if (points[i][1] < points[bottom_idx][1] or 
                (points[i][1] == points[bottom_idx][1] and points[i][0] < points[bottom_idx][0])):
                bottom_idx = i
        ...
        sorted_points = sorted(points[1:], 
            key=lambda p: (math.atan2(p[1] - p0[1], p[0] - p0[0]),
    ...
    def evaluate_rational_bezier_variable(self, control_points, t):
        ...
        for i, cp in enumerate(control_points):
            ...
            weight = cp[2] if len(cp) > 2 else 1.0  # Use weight if provided, otherwise 1.0
    ...
    def evaluate_bezier_curve(self, control_points, t):
        ...
        points = [(p[0], p[1]) for p in control_points]  # Ensure we only use x,y coordinates
    ...
    def initialize_control_points(self, edge, source_node, target_node):
        ...
        if rendering_type == "straight":
            ...
            if is_self_loop:
                ...
                edge.control_points = [(source_node.x + 30, source_node.y - 50, 0.0)]
            else:
            ...
            if is_self_loop:
                ...
                edge.control_points = [
                    (source_node.x + 40, source_node.y - 30, 0.0),  # First control point
                    (source_node.x - 40, source_node.y - 30, 0.0)   # Second control point
                ]
            else:
            ...
            if is_self_loop:
                ...
                edge.control_points = [
                    (source_node.x + 50, source_node.y - 20, 0.0),  # Right and up
                    (source_node.x + 20, source_node.y - 60, 0.0),  # Above
                    (source_node.x - 30, source_node.y - 20, 0.0)   # Left and up
                ]
            else:
                # Start with 0 control points - user can add them manually
                edge.control_points = []
            ...
            if not getattr(edge, 'curve_segments', None):
                ...
                edge.curve_segments = [{"type": "bezier", "control_points": [(cp1_x, cp1_y), (cp2_x, cp2_y)], "weight": 1.0}]
        
        edge.custom_position = len(edge.control_points) > 0
        print(f"DEBUG: 🎛️ Initialized {len(edge.control_points)} control points for edge {edge.id} (type: {rendering_type}): {edge.control_points}")
    ...
    def get_control_point_at_position(self, screen_x, screen_y):
        ...
        if not self.control_points_enabled:
            print(f"DEBUG: 🎛️ Control points disabled, not checking hit detection")
            return None
            
        control_radius = max(10, int(8 * self.zoom))  # Increased radius and better scaling
        print(f"DEBUG: 🎛️ Hit testing control points at screen({screen_x}, {screen_y}) with radius {control_radius}")
        ...
        try:
            ...
            for edge in self.graph.get_all_edges():
                ...
                for i, control_point in enumerate(edge.control_points):
                    ...
                    screen_pos = self.world_to_screen(control_point[0], control_point[1])
                    distance = math.sqrt((test_x - screen_pos[0])**2 + (test_y - screen_pos[1])**2)
                        
                    if distance <= control_radius:
                        print(f"DEBUG: 🎛️ Hit control point {i} of edge {edge.id} at distance {distance:.1f}")
                        return (edge, i)
        ...
        for edge in self.graph.get_all_edges():
            ...
            rendering_type = edge.rendering_type if edge.rendering_type else self.edge_rendering_type
            print(f"DEBUG: 🎛️ Checking edge {edge.id} (type: {rendering_type})")
            ...
            if rendering_type == "composite" and getattr(edge, 'is_composite', False):
                ...
                if hasattr(edge, 'curve_segments') and segment_index < len(edge.curve_segments):
                    ...
                    print(f"DEBUG: 🔗 Hit testing {len(control_points_to_check)} composite segment {segment_index} control points")
                    for i, cp in enumerate(control_points_to_check):
                        ...
                        print(f"DEBUG: 🔗 Composite CP {i}: world({cp[0]:.1f}, {cp[1]:.1f}) -> screen({cp_screen[0]:.1f}, {cp_screen[1]:.1f}), distance: {dist:.1f}")
                        if dist <= control_radius:
                            print(f"DEBUG: 🎛️ ✅ HIT composite control point {i} of edge {edge.id}")
                            return (edge, i)
            else:
                # For regular curves, use the edge's control points
                control_points_to_check = edge.control_points if hasattr(edge, 'control_points') else []
                print(f"DEBUG: 🎛️ Edge {edge.id} has {len(control_points_to_check)} control points: {control_points_to_check}")
                ...
                for i, control_point in enumerate(control_points_to_check):
                    ...
                    print(f"DEBUG: 🎛️ CP {i}: world({control_point[0]:.1f}, {control_point[1]:.1f}) -> screen({screen_pos[0]:.1f}, {screen_pos[1]:.1f}), distance: {distance:.1f}")
                    
                    if distance <= control_radius:
                        print(f"DEBUG: 🎛️ ✅ HIT control point {i} of {rendering_type} edge {edge.id} at distance {distance:.1f}")
                        return (edge, i)
    ...
    def get_arrow_position_control_at_position(self, screen_x, screen_y):
        ...
        for edge in self.graph.get_all_edges():
            ...
            source_adjusted = self.calculate_line_endpoint(source_node, target_node,
                                                          source_screen, target_screen, True, edge)
            target_adjusted = self.calculate_line_endpoint(target_node, source_node,
                                                          target_screen, source_screen, False, edge)
            
            if edge.is_hyperedge:
                # Calculate arrow position based on the main segment
                ...
                # Calculate arrow position within the main segment
    ...
    def calculate_curve_tangent_at_target(self, edge, source_node, target_node):
        ...
        try:
            ...
            elif rendering_type in ["bezier", "cubic_spline", "nurbs"] and len(edge.control_points) >= 2:
            ...
            elif rendering_type == "polyline" and len(edge.control_points) >= 1:
            ...
            print(f"DEBUG: ⚠️ Error calculating curve tangent at target: {e}, using fallback")
            
            # Fallback to straight line direction
            return target_node.x - source_node.x, target_node.y - source_node.y
    
    def calculate_curve_tangent_at_position(self, edge, source_node, target_node, t):
        """Calculate the tangent direction at any position t along the curve."""

        rendering_type = edge.rendering_type if edge.rendering_type else self.edge_rendering_type
        print(f"DEBUG: 🧭 Calculating tangent for {rendering_type} edge {edge.id} at t={t:.3f}")
        ...
        if rendering_type == "straight" or not edge.control_points:
            ...
            if (hasattr(edge, 'control_points') and edge.control_points and 
                ...
                try:
                    ...
                    cp_x, cp_y = cp[0], cp[1]
                    dx = 2*(1-t)*(cp_x - source_node.x) + 2*t*(target_node.x - cp_x)
                    dy = 2*(1-t)*(cp_y - source_node.y) + 2*t*(target_node.y - cp_y)
                    print(f"DEBUG: 🧭 Curved tangent using CP({cp_x:.1f}, {cp_y:.1f}): ({dx:.2f}, {dy:.2f})")
                    return dx, dy
                except (IndexError, TypeError, ValueError) as e:
                    print(f"DEBUG: ⚠️ Error accessing curved control point: {e}, falling back to straight line")
            
                    # Fallback to straight line if no valid control point
                    print(f"DEBUG: 🧭 Curved edge has no valid control points, using straight line tangent")
                    dx, dy = target_node.x - source_node.x, target_node.y - source_node.y
                    print(f"DEBUG: 🧭 Straight line fallback tangent: ({dx:.2f}, {dy:.2f})")
            return dx, dy
            ...
            if (hasattr(edge, 'control_points') and edge.control_points and 
                ...
                try:
                    ctrl1_x, ctrl1_y = edge.control_points[0][0], edge.control_points[0][1]
                    ctrl2_x, ctrl2_y = edge.control_points[1][0], edge.control_points[1][1]
                    print(f"DEBUG: 🧭 Using actual control points: CP1({ctrl1_x:.1f}, {ctrl1_y:.1f}), CP2({ctrl2_x:.1f}, {ctrl2_y:.1f})")
                except (IndexError, TypeError, ValueError) as e:
                    print(f"DEBUG: ⚠️ Error accessing control points: {e}, falling back to defaults")
                ...
                print(f"DEBUG: 🧭 Insufficient control points (has {len(edge.control_points) if hasattr(edge, 'control_points') and edge.control_points else 0}), using defaults")
                dx = target_node.x - source_node.x
                dy = target_node.y - source_node.y
                ctrl1_x = source_node.x + dx * 0.3
                ctrl1_y = source_node.y + dy * 0.3 - 30
            ...
            dx = (3*(1-t)**2 * (ctrl1_x - source_node.x) + 
                  6*(1-t)*t * (ctrl2_x - ctrl1_x) + 
                  3*t**2 * (target_node.x - ctrl2_x))
            dy = (3*(1-t)**2 * (ctrl1_y - source_node.y) + 
                  6*(1-t)*t * (ctrl2_y - ctrl1_y) + 
                  3*t**2 * (target_node.y - ctrl2_y))
            print(f"DEBUG: 🧭 {rendering_type} tangent using CPs({ctrl1_x:.1f}, {ctrl1_y:.1f}), ({ctrl2_x:.1f}, {ctrl2_y:.1f}): ({dx:.2f}, {dy:.2f})")
            return dx, dy
            ...
            p1 = self.calculate_position_on_curve(edge, source_node, target_node, t1)
            p2 = self.calculate_position_on_curve(edge, source_node, target_node, t2)
        ...
        elif rendering_type == "composite" or getattr(edge, 'is_composite', False):
    ...
    def set_edge_curve_type(self, edge, curve_type):
        ...
        requires_specific_init = curve_type in ["curved"]  # Only arcs require specific control points
        
        if not preserve_custom or requires_specific_init:
            print(f"DEBUG: 🔄 Clearing control points for curve type change to {curve_type} (preserve_custom: {preserve_custom}, requires_specific: {requires_specific_init})")
        edge.control_points = []
        edge.custom_position = False
        ...
        if source_node and target_node:
            ...
            print(f"DEBUG: 🔄 PRESERVING {len(edge.control_points)} custom control points when changing to {curve_type}")
            print(f"DEBUG: 🎛️ Re-initialized control points for edge {edge.id} with type {curve_type}: {edge.control_points}")
    ...
    def add_curve_control_point(self, world_x, world_y, insert_index=None):
        ...
        print(f"DEBUG: 🌊 add_curve_control_point called with ({world_x}, {world_y}), editing_edge = {self.bspline_editing_edge}")
        ...
        if not hasattr(self.bspline_editing_edge, 'control_points'):
            print(f"DEBUG: 🌊 Edge {self.bspline_editing_edge.id} has no control_points attribute - initializing")
            self.bspline_editing_edge.control_points = []
        
        print(f"DEBUG: 🌊 Edge {self.bspline_editing_edge.id} currently has {len(self.bspline_editing_edge.control_points)} control points")
        
        if insert_index is None:
            # Add at the end - use 3D coordinates (x, y, z) as expected by Edge model
            self.bspline_editing_edge.control_points.append((world_x, world_y, 0.0))
            ...
            self.bspline_editing_edge.control_points.insert(insert_index, (world_x, world_y, 0.0))
        
        self.bspline_editing_edge.custom_position = True
        
        # Get curve type for logging
        curve_type = getattr(self.bspline_editing_edge, 'rendering_type', None) or self.edge_rendering_type
        print(f"DEBUG: 🌊 Added {curve_type} control point at ({world_x:.1f}, {world_y:.1f})")
        print(f"DEBUG: 🌊 {curve_type} now has {len(self.bspline_editing_edge.control_points)} control points")
        ...
        elif hasattr(self.GetParent(), 'update_bspline_list'):  # Fallback for compatibility
    ...
    def delete_bspline_control_point(self, index):
        ...
        if not self.bspline_editing_edge or index < 0 or index >= len(self.bspline_editing_edge.control_points):
            return
        
        deleted_point = self.bspline_editing_edge.control_points.pop(index)
        self.bspline_editing_edge.custom_position = True
        print(f"DEBUG: 🌊 Deleted B-spline control point at index {index}: {deleted_point}")
        self.graph_modified.emit()
    ...
    def move_bspline_control_point(self, from_index, to_index):
        ...
        if not self.bspline_editing_edge or from_index < 0 or to_index < 0:
            return
        if from_index >= len(self.bspline_editing_edge.control_points) or to_index >= len(self.bspline_editing_edge.control_points):
            return
        
        # Move the control point
        control_point = self.bspline_editing_edge.control_points.pop(from_index)
        self.bspline_editing_edge.control_points.insert(to_index, control_point)
        self.bspline_editing_edge.custom_position = True
        print(f"DEBUG: 🌊 Moved B-spline control point from index {from_index} to {to_index}")
        self.graph_modified.emit()
        # Update the main window's B-spline list if it exists
        if hasattr(self.GetParent(), 'update_bspline_list'):
            wx.CallAfter(self.GetParent().update_bspline_list)
    ...
    def start_adding_bspline_control_point(self):
        ...
        print(f"DEBUG: 🌊 Started adding B-spline control point mode - click to add points")
    ...
    def show_bspline_control_points_dialog(self, edge):
        ...
        if not edge or not edge.control_points:
            wx.MessageBox("No control points to display.", "B-spline Control Points", wx.OK | wx.ICON_INFORMATION)
            return
        
        # Create a simple dialog showing control points
        dialog = wx.Dialog(self, title=f"B-spline Control Points (Edge {edge.id})", size=(400, 300))
        sizer = wx.BoxSizer(wx.VERTICAL)
        
        # Instructions
        instructions = wx.StaticText(dialog, label="Control Points (excluding source and target nodes):")
        ...
        def on_add_point(evt):
            ...
            coord_dialog = wx.TextEntryDialog(dialog, "Enter coordinates (x,y):", "Add Control Point", "0,0")
            if coord_dialog.ShowModal() == wx.ID_OK:
                try:
                    ...
                    wx.MessageBox("Invalid coordinates format. Use: x,y", "Error", wx.OK | wx.ICON_ERROR)
        ...
        def on_delete_point(evt):
            ...
            if selected >= 0:
                ...
                wx.MessageBox("Please select a control point to delete.", "No Selection", wx.OK | wx.ICON_WARNING)
    ...
    def calculate_position_on_curve(self, edge, source_node, target_node, t):
        ...
        needs_init = (
            (rendering_type == "curved" and len(edge.control_points) == 0) or  # Arcs need 1 control point
            (source_node.id == target_node.id and len(edge.control_points) == 0)  # Self-loops need control points
        )
        if needs_init:
            print(f"DEBUG: 🎛️ Auto-initializing control points for {rendering_type} edge (self-loop: {source_node.id == target_node.id})")
            self.initialize_control_points(edge, source_node, target_node)
        ...
        if rendering_type == "straight":
            ...
            if edge.freeform_points and len(edge.freeform_points) >= 2:
                ...
                for i in range(len(segment_lengths)):
                    if current_distance + segment_lengths[i] >= target_distance:
                        ...
                        p1 = edge.freeform_points[i]
                        p2 = edge.freeform_points[i + 1]
                        segment_t = (target_distance - current_distance) / segment_lengths[i]
            ...
            if edge.control_points and len(edge.control_points) >= 1:
                ...
                if 0 < segment_index < segments - 1:
                    ...
                    base_x += offset * math.cos(math.atan2(dy, dx) + math.pi/2)
                    base_y += offset * math.sin(math.atan2(dy, dx) + math.pi/2)
            ...
            segment_points = [(source_node.x, source_node.y)]  # Start with source
            ...
            segment_points.append((target_node.x, target_node.y))  # End with target
            ...
            return self.calculate_segment_position(segment, start_pos, end_pos, local_t)
    ...
    def find_nearest_curve_parameter(self, edge, source_node, target_node, target_point):
        ...
        source_anchor_world, target_anchor_world = self.get_edge_anchor_endpoints_world(edge, source_node, target_node)
        print(f"DEBUG: 🟡 Finding nearest curve parameter using anchor endpoints: source {source_anchor_world}, target {target_anchor_world}")
        ...
        for i in range(101):
            t = i / 100.0
            curve_point = self.calculate_position_on_curve_with_anchors(edge, source_anchor_world, target_anchor_world, t)
        ...
        print(f"DEBUG: 🟡 Found nearest curve parameter t={best_t:.3f} at distance {best_distance:.1f}")
        return best_t
        
    def _point_to_line_distance(self, px: float, py: float, p1: tuple, p2: tuple) -> float:
        """Calculate the distance from a point to a line segment."""
    ...
    def get_edge_at_position(self, screen_x, screen_y):
        ...
        print(f"DEBUG: 🎯 Edge hit test at screen({screen_x}, {screen_y}) with tolerance {click_tolerance} (rotation: {self.world_rotation}°)")
        ...
        for edge in self.graph.get_all_edges():
            if edge.is_hyperedge and edge.hyperedge_visualization == "ubergraph":
                ...
                edge_screen_x, edge_screen_y = self.world_to_screen(edge.uber_x, edge.uber_y)
                
                # Check if click is within the edge's bounds
                half_width = edge.uber_width / 2
                half_height = edge.uber_height / 2
                
                if (abs(test_x - edge_screen_x) <= half_width and 
                    abs(test_y - edge_screen_y) <= half_height):
                    print(f"DEBUG: 🎯 Hit ubergraph edge {edge.id}")
                    return edge
            
            print(f"DEBUG: 🎯 Inverse rotation: screen({screen_x}, {screen_y}) -> pre-rotation({test_x:.1f}, {test_y:.1f})")
        ...
        for edge in self.graph.get_all_edges():
            ...
            if edge.is_hyperedge:
                ...
                for node in [source_node] + [self.graph.get_node(nid) for nid in edge.source_ids]:
                    if not node:
                        continue
                    node_screen = self.world_to_screen(node.x, node.y)
                    node_adjusted = self.calculate_line_endpoint(node, source_node,
                                                             node_screen, source_screen, True, edge)
                    dist = self._point_to_line_distance(test_x, test_y, node_adjusted, from_point)
                    if dist <= click_tolerance and dist < best_distance:
                        best_distance = dist
                        best_edge = edge
                
                # Check target node connections
                for node in [target_node] + [self.graph.get_node(nid) for nid in edge.target_ids]:
                    if not node:
                        continue
                    node_screen = self.world_to_screen(node.x, node.y)
                    node_adjusted = self.calculate_line_endpoint(node, target_node,
                                                             node_screen, target_screen, True, edge)
                    dist = self._point_to_line_distance(test_x, test_y, node_adjusted, to_point)
                    if dist <= click_tolerance and dist < best_distance:
                        best_distance = dist
                        best_edge = edge
                
                # Check main segment
                dist = self._point_to_line_distance(test_x, test_y, from_point, to_point)
                if dist <= click_tolerance and dist < best_distance:
                    best_distance = dist
                    best_edge = edge
            else:
                # Regular edge - check the main line
                dist = self._point_to_line_distance(test_x, test_y, source_adjusted, target_adjusted)
                if dist <= click_tolerance and dist < best_distance:
                    best_distance = dist
                    best_edge = edge
        ...
        for edge in self.graph.get_all_edges():
            if not hasattr(edge, 'visible') or edge.visible:
                ...
                source_adjusted = self.calculate_line_endpoint(source_node, target_node, source_screen, target_screen, True, edge)
                target_adjusted = self.calculate_line_endpoint(target_node, source_node, target_screen, source_screen, False, edge)
                ...
                if edge.rendering_type == "freeform" and edge.freeform_points and len(edge.freeform_points) >= 2:
                    for i in range(len(edge.freeform_points) - 1):
                        p1 = edge.freeform_points[i]
                        p2 = edge.freeform_points[i + 1]
                        ...
                        if length_sq == 0:
                            # Point case
                            distance = math.sqrt((test_x - p1_screen[0])**2 + (test_y - p1_screen[1])**2)
                        else:
                            # Calculate projection parameter
                            t = max(0, min(1, ((test_x - p1_screen[0]) * dx + (test_y - p1_screen[1]) * dy) / length_sq))
                            ...
                            distance = math.sqrt((test_x - proj_x)**2 + (test_y - proj_y)**2)
                ...
                for i in range(20):  # Sample at 20 points along the curve
                    ...
                    if hasattr(edge, 'control_points') and hasattr(edge, 'rendering_type'):
                        rendering_type = edge.rendering_type if edge.rendering_type else self.edge_rendering_type
                        if rendering_type != 'straight' and edge.control_points:
                            # Use the same curve calculation as the drawing code
                            source_anchor_world, target_anchor_world = self.get_edge_anchor_endpoints_world(edge, source_node, target_node)
                            curve_world = self.calculate_position_on_curve_with_anchors(edge, source_anchor_world, target_anchor_world, t)
                            if curve_world:
                                curve_screen = self.world_to_screen(curve_world[0], curve_world[1])
                                curve_x, curve_y = curve_screen[0], curve_screen[1]
                            else:
                                # Fallback to straight line
                                curve_x = source_adjusted[0] + t * (target_adjusted[0] - source_adjusted[0])
                                curve_y = source_adjusted[1] + t * (target_adjusted[1] - source_adjusted[1])
                        else:
                            # Straight line case
                            curve_x = source_adjusted[0] + t * (target_adjusted[0] - source_adjusted[0])
                            curve_y = source_adjusted[1] + t * (target_adjusted[1] - source_adjusted[1])
                ...
                if min_distance < click_tolerance and min_distance < best_distance:
                    ...
                    print(f"DEBUG: 🎯 Edge {edge.id} hit at distance {min_distance:.1f} pixels")
        
        if best_edge:
            print(f"DEBUG: 🎯 ✅ Selected edge {best_edge.id} at distance {best_distance:.1f}")
            return best_edge
        else:
    ...
    def get_edge_endpoint_at_position(self, screen_x, screen_y):
        ...
        for edge in self.graph.get_all_edges():
            ...
            if edge.is_hyperedge:
                # Check source node connections
                for i, node in enumerate([source_node] + [self.graph.get_node(nid) for nid in edge.source_ids]):
                    if not node:
                        continue
                    node_screen = self.world_to_screen(node.x, node.y)
                    node_adjusted = self.calculate_line_endpoint(node, source_node,
                                                             node_screen, source_screen, True, edge)
                    dist = math.sqrt((test_x - node_adjusted[0])**2 + (test_y - node_adjusted[1])**2)
                    if dist <= dot_radius:
                        return (edge, f'source_{i}')
                
                # Check target node connections
                for i, node in enumerate([target_node] + [self.graph.get_node(nid) for nid in edge.target_ids]):
                    if not node:
                        continue
                    node_screen = self.world_to_screen(node.x, node.y)
                    node_adjusted = self.calculate_line_endpoint(node, target_node,
                ...
                source_dist = math.sqrt((test_x - source_adjusted[0])**2 + (test_y - source_adjusted[1])**2)
                if source_dist <= dot_radius:
                    return (edge, 'source')
                
                # Check target dot (using inverse-rotated coordinates)
                target_dist = math.sqrt((test_x - target_adjusted[0])**2 + (test_y - target_adjusted[1])**2)
    ...
    def get_edge_anchor_endpoints_world(self, edge, source_node, target_node):
        ...
        source_adjusted_screen = self.calculate_line_endpoint(source_node, target_node, 
                                                            source_screen, target_screen, True, edge)
        target_adjusted_screen = self.calculate_line_endpoint(target_node, source_node,
                                                            target_screen, source_screen, False, edge)
        
        # Convert back to world coordinates
        source_anchor_world = self.screen_to_world(source_adjusted_screen[0], source_adjusted_screen[1])
        target_anchor_world = self.screen_to_world(target_adjusted_screen[0], target_adjusted_screen[1])
    ...
    def debug_check_node_visibility(self, context=""):
        ...
        if invisible_nodes:
            print(f"DEBUG: 👻 {context} - Found invisible nodes: {invisible_nodes}")
        else:
    ...
    def calculate_position_on_curve_with_anchors(self, edge, source_anchor, target_anchor, t):
        ...
        if rendering_type == "freeform" and edge.freeform_points and len(edge.freeform_points) >= 2:
            # For freeform edges, interpolate along the path points
            total_length = 0
            segment_lengths = []
            for i in range(len(edge.freeform_points) - 1):
            ...
            if (hasattr(edge, 'control_points') and edge.control_points and 
                ...
                try:
                    ...
                    return self.evaluate_quadratic_bezier_curve(
                        [source_anchor, (control_point[0], control_point[1]), target_anchor], t)
            ...
            if length > 0:
                ...
                return self.evaluate_quadratic_bezier_curve([source_anchor, control_point, target_anchor], t)
            else:
            ...
            if (hasattr(edge, 'control_points') and edge.control_points and 
                ...
                try:
                    ...
                    knot_points = [source_anchor] + edge.control_points + [target_anchor]
            ...
            if (hasattr(edge, 'control_points') and edge.control_points and 
                ...
                try:
                    ...
                    control_points = [source_anchor] + edge.control_points + [target_anchor]
                    return self.evaluate_bezier_curve(control_points, t)
                except (IndexError, TypeError, ValueError):
                    pass
            ...
            if (hasattr(edge, 'control_points') and edge.control_points and 
                ...
                try:
                    ...
                    points = [source_anchor] + edge.control_points + [target_anchor]
                    return self.evaluate_interpolating_spline(points, t)
                except (IndexError, TypeError, ValueError):
                    pass
            ...
            if (hasattr(edge, 'control_points') and edge.control_points and 
                ...
                try:
                    ...
                    weighted_points = [(source_anchor[0], source_anchor[1], 1.0)]
                    for cp in edge.control_points:
                        weighted_points.append((cp[0], cp[1], 1.0))
                    weighted_points.append((target_anchor[0], target_anchor[1], 1.0))
                    return self.evaluate_rational_bezier_curve(weighted_points, t)
                except (IndexError, TypeError, ValueError):
                    pass
            ...
            if (hasattr(edge, 'control_points') and edge.control_points and 
                ...
                try:
                    ...
                    segment_points = [source_anchor] + edge.control_points + [target_anchor]
            ...
            if hasattr(edge, 'curve_segments') and edge.curve_segments:
                try:
                    ...
                    segment_start_x = source_anchor[0] + segment_start_t * (target_anchor[0] - source_anchor[0])
                    segment_start_y = source_anchor[1] + segment_start_t * (target_anchor[1] - source_anchor[1])
                    segment_end_x = source_anchor[0] + segment_end_t * (target_anchor[0] - source_anchor[0])
                    segment_end_y = source_anchor[1] + segment_end_t * (target_anchor[1] - source_anchor[1])
                    ...
                    if segment_type == "bezier" and len(segment_control_points) >= 1:
                        # Create full control points list for this segment
                        control_points = [segment_start] + segment_control_points + [segment_end]
                        return self.evaluate_bezier_curve(control_points, segment_t)
                    elif segment_type == "bspline" and len(segment_control_points) >= 1:
                        knot_points = [segment_start] + segment_control_points + [segment_end]
                        return self.evaluate_bspline_variable(knot_points, segment_t)
                    elif segment_type == "cubic_spline" and len(segment_control_points) >= 1:
                        points = [segment_start] + segment_control_points + [segment_end]
                        return self.evaluate_interpolating_spline(points, segment_t)
                    else:
                        # Default to linear interpolation for this segment
                        x = segment_start[0] + segment_t * (segment_end[0] - segment_start[0])
                        y = segment_start[1] + segment_t * (segment_end[1] - segment_start[1])
                        return (x, y)
                        
                except (IndexError, TypeError, ValueError) as e:
                    print(f"DEBUG: ⚠️ Error calculating composite curve position: {e}")
    ...
    def calculate_curve_tangent_at_position_with_anchors(self, edge, source_anchor, target_anchor, t):
        ...
        try:
            if rendering_type == "straight":
                ...
                if (hasattr(edge, 'control_points') and edge.control_points and 
                    ...
                    try:
                        ...
                        control_point = edge.control_points[0]
                        p0, p1, p2 = source_anchor, (control_point[0], control_point[1]), target_anchor
                ...
                if (hasattr(edge, 'control_points') and edge.control_points and 
                    ...
                    try:
                        ...
                        dt = 0.01
                        p1 = self.evaluate_bspline_variable(knot_points, max(0, t - dt/2))
                        p2 = self.evaluate_bspline_variable(knot_points, min(1, t + dt/2))
                        dx = (p2[0] - p1[0]) / dt
                        dy = (p2[1] - p1[1]) / dt
                        return (dx, dy)
                    except (IndexError, TypeError, ValueError):
                ...
                if (hasattr(edge, 'control_points') and edge.control_points and 
                    ...
                    try:
                        ...
                        dx, dy = 0, 0
                        for i in range(n):
                            coeff = self.binomial_coefficient(n-1, i) * ((1-t)**(n-1-i)) * (t**i)
                            dx += coeff * (control_points[i+1][0] - control_points[i][0])
                            dy += coeff * (control_points[i+1][1] - control_points[i][1])
                ...
                if (hasattr(edge, 'control_points') and edge.control_points and 
                    ...
                    try:
                        ...
                        dt = 0.01
                        p1 = self.evaluate_interpolating_spline(points, max(0, t - dt/2))
                        p2 = self.evaluate_interpolating_spline(points, min(1, t + dt/2))
                        dx = (p2[0] - p1[0]) / dt
                        dy = (p2[1] - p1[1]) / dt
                        return (dx, dy)
                    except (IndexError, TypeError, ValueError):
                ...
                try:
                    dt = 0.01
                    p1 = self.calculate_position_on_curve_with_anchors(edge, source_anchor, target_anchor, max(0, t - dt/2))
                    p2 = self.calculate_position_on_curve_with_anchors(edge, source_anchor, target_anchor, min(1, t + dt/2))
                    dx = (p2[0] - p1[0]) / dt
                    dy = (p2[1] - p1[1]) / dt
                    return (dx, dy)
                except:
            ...
            print(f"DEBUG: ⚠️ Error calculating tangent for {rendering_type}: {e}")
    ...
    def on_left_down(self, event):
        ...
        if self.tool == "select":
            ...
            if self.adding_bspline_control_point:
                ...
                print(f"DEBUG: 🌊 Added control point, staying in click mode (use 'End Click' to exit)")
                return
            ...
            print(f"DEBUG: 🔄 ROTATE WORLD TOOL DETECTED - calling handle_rotate_down")
            self.handle_rotate_down(pos)
        elif self.tool == "rotate_element":
            print(f"DEBUG: 🔄 ROTATE ELEMENT TOOL DETECTED in on_left_down - calling handle_rotate_element_down")
            self.handle_rotate_element_down(pos)
        elif self.tool == "drag_into":
            print(f"DEBUG: 📦 ============ DRAG INTO CONTAINER TOOL DETECTED ============")
            print(f"DEBUG: 📦 Calling handle_drag_into_down at {pos.x}, {pos.y}")
            self.handle_drag_into_down(pos)
        elif self.tool == "expand":
            print(f"DEBUG: 📖 EXPAND TOOL DETECTED - calling handle_expand_down")
            self.handle_expand_down(pos)
        elif self.tool == "collapse":
            print(f"DEBUG: 📕 COLLAPSE TOOL DETECTED - calling handle_collapse_down")
            self.handle_collapse_down(pos)
        elif self.tool == "recursive_expand":
            print(f"DEBUG: 🔄📖 RECURSIVE EXPAND TOOL DETECTED - calling handle_recursive_expand_down")
            self.handle_recursive_expand_down(pos)
        elif self.tool == "recursive_collapse":
            print(f"DEBUG: 🔄📕 RECURSIVE COLLAPSE TOOL DETECTED - calling handle_recursive_collapse_down")
    ...
    def on_left_up(self, event):
        ...
        if self.tool == "select":
            print(f"DEBUG: 🔗 Select tool up - dragging: {self.dragging}, drag_offset count: {len(self.drag_offset) if self.drag_offset else 0}")
            ...
            print(f"DEBUG: 📦 ============ DRAG INTO CONTAINER UP DETECTED ============")
            print(f"DEBUG: 📦 About to call handle_drag_into_up with pos ({pos.x}, {pos.y})")
        ...
        print(f"DEBUG: 🔗 MOUSE UP CHECK - dragging: {self.dragging}, drag_offset: {len(self.drag_offset) if self.drag_offset else 0}, grid_snapping_enabled: {self.grid_snapping_enabled}")

        # Apply grid snapping to dragged nodes if dragging was active
        if self.dragging and self.drag_offset:
            print(f"DEBUG: 🔗 Applying grid snapping to {len(self.drag_offset)} dragged nodes (enabled: {self.grid_snapping_enabled})")
            ...
            for node_id in self.drag_offset.keys():
                node = self.graph.get_node(node_id)
                if node:
                    print(f"DEBUG: 🔗 Node {node_id} before snap: ({node.x:.1f}, {node.y:.1f})")
                    snapped_pos = self.snap_to_grid(node.x, node.y, node.width, node.height)
                    node.set_2d_position(snapped_pos[0], snapped_pos[1])
                    print(f"DEBUG: 🔗 Node {node_id} after snap: ({node.x:.1f}, {node.y:.1f})")
        ...
        self.dragging_arrow_position = False
        self.dragging_arrow_position_edge = None
        print(f"DEBUG: 📦 FINAL CLEANUP - Clearing drag states in on_left_up - was dragging: {self.dragging_into_container}")
        if self.dragging_into_container:
            print(f"DEBUG: 📦 ⚠️ WARNING: Clearing dragging_into_container without completing containment!")
        self.dragging_into_container = False
    ...
    def on_right_down(self, event):
        ...
        pos = event.GetPosition()
        print(f"DEBUG: Right button down at ({pos.x}, {pos.y})")
        ...
        if node:
            ...
            if edge_type == "bspline":
                ...
                menu.Bind(wx.EVT_MENU, lambda evt: self.enter_bspline_editing_mode(edge), id=20)
                menu.Bind(wx.EVT_MENU, lambda evt: self.exit_bspline_editing_mode(), id=21)
                menu.Bind(wx.EVT_MENU, lambda evt: self.start_adding_bspline_control_point(), id=22)
                menu.Bind(wx.EVT_MENU, lambda evt: self.show_bspline_control_points_dialog(edge), id=23)
            ...
            menu.Bind(wx.EVT_MENU, lambda evt: self.set_edge_curve_type(edge, "straight"), id=10)
            menu.Bind(wx.EVT_MENU, lambda evt: self.set_edge_curve_type(edge, "curved"), id=11)
            menu.Bind(wx.EVT_MENU, lambda evt: self.set_edge_curve_type(edge, "bezier"), id=12)
            menu.Bind(wx.EVT_MENU, lambda evt: self.set_edge_curve_type(edge, "bspline"), id=13)
            menu.Bind(wx.EVT_MENU, lambda evt: self.set_edge_curve_type(edge, "cubic_spline"), id=14)
            menu.Bind(wx.EVT_MENU, lambda evt: self.set_edge_curve_type(edge, "nurbs"), id=15)
            menu.Bind(wx.EVT_MENU, lambda evt: self.set_edge_curve_type(edge, "polyline"), id=16)
            menu.Bind(wx.EVT_MENU, lambda evt: self.set_edge_curve_type(edge, None), id=17)
        else:
            ...
            if selected_nodes or selected_edges:
                ...
                menu.Bind(wx.EVT_MENU, lambda evt: self.on_delete_selection(), id=9)
            
            menu.Append(7, "Add Node Here")
            menu.AppendSeparator()
            menu.Append(8, "Paste")
            # Bind general menu events
            menu.Bind(wx.EVT_MENU, lambda evt: self.on_add_node_here(pos), id=7)
    ...
    def on_delete_selection(self):
        ...
        if self.undo_redo_manager:
            ...
            for edge in selected_edges:
                commands.append(m_commands.DeleteEdgeCommand(self.graph, edge.id))
            
            selected_nodes = self.graph.get_selected_nodes()
            for node in selected_nodes:
                commands.append(m_commands.DeleteNodeCommand(self.graph, node.id))
            
            if commands:
                composite_command = m_commands.CompositeCommand("Delete Selection", commands)
    ...
    def handle_node_creation(self, pos):
        ...
        world_pos = self.screen_to_world(pos.x, pos.y)
        print(f"DEBUG: 🖱️ MOUSE NODE PLACEMENT: screen=({pos.x}, {pos.y}) -> world={world_pos}")
    ...
    def on_add_node_here(self, pos):
        ...
        print(f"DEBUG: 📍 Add node here at world_pos: {world_pos}, grid_snapping_enabled: {self.grid_snapping_enabled}")
        if getattr(self, 'grid_snapping_enabled', False):
            snapped_pos = self.snap_to_grid_position(world_pos[0], world_pos[1])
        else:
    ...
    def get_tool_from_main_window(self):
        ...
        try:
            ...
            if hasattr(main_window, 'tool_select') and main_window.tool_select.GetValue():
                return "select"
            elif hasattr(main_window, 'tool_node') and main_window.tool_node.GetValue():
                return "node"
            elif hasattr(main_window, 'tool_edge') and main_window.tool_edge.GetValue():
                return "edge"
            elif hasattr(main_window, 'tool_move') and main_window.tool_move.GetValue():
                return "move"
            elif hasattr(main_window, 'tool_rotate') and main_window.tool_rotate.GetValue():
                print(f"DEBUG: 🎯 ROTATE WORLD TOOL IS SELECTED!")
                return "rotate"
            elif hasattr(main_window, 'tool_rotate_element') and main_window.tool_rotate_element.GetValue():
                print(f"DEBUG: 🎯 ROTATE ELEMENT TOOL IS SELECTED!")
                return "rotate_element"
            elif hasattr(main_window, 'tool_drag_into') and main_window.tool_drag_into.GetValue():
                print(f"DEBUG: 📦 DRAG INTO CONTAINER TOOL IS SELECTED!")
                return "drag_into"
            elif hasattr(main_window, 'tool_expand') and main_window.tool_expand.GetValue():
                print(f"DEBUG: 📖 EXPAND TOOL IS SELECTED!")
                return "expand"
            elif hasattr(main_window, 'tool_collapse') and main_window.tool_collapse.GetValue():
                print(f"DEBUG: 📕 COLLAPSE TOOL IS SELECTED!")
                return "collapse"
            elif hasattr(main_window, 'tool_recursive_expand') and main_window.tool_recursive_expand.GetValue():
                print(f"DEBUG: 🔄📖 RECURSIVE EXPAND TOOL IS SELECTED!")
                return "recursive_expand"
            elif hasattr(main_window, 'tool_recursive_collapse') and main_window.tool_recursive_collapse.GetValue():
    ...
    def on_motion(self, event):
        ...
        if self.tool == "select":
            ...
            print(f"DEBUG: 🔄 ROTATE WORLD MOTION - calling handle_rotate_motion at {pos.x}, {pos.y}")
            self.handle_rotate_motion(pos)
        elif self.tool == "rotate_element":
            print(f"DEBUG: 🔄 ROTATE ELEMENT MOTION - calling handle_rotate_element_motion at {pos.x}, {pos.y}")
    ...
    def on_right_down(self, event):
        ...
        view_menu.Append(wx.ID_ANY, f"Center: ({world_center_x:.1f}, {world_center_y:.1f})")
        view_menu.Append(wx.ID_ANY, f"World Size: {world_width:.1f} x {world_height:.1f}")
        view_menu.Append(wx.ID_ANY, f"Screen Size: {size.width} x {size.height}")
        ...
        if clicked_node:
            ...
            self.Bind(wx.EVT_MENU, lambda evt: self.delete_node_context_menu(clicked_node.id), id=delete_node_id)
            ...
            self.Bind(wx.EVT_MENU, lambda evt: self.delete_edge_context_menu(clicked_edge.id), id=delete_edge_id)
            ...
            if self.has_selected_items():
                ...
                self.Bind(wx.EVT_MENU, lambda evt: self.delete_selected_context_menu(), id=delete_selected_id)
    ...
    def delete_selected_context_menu(self):
        ...
        selected_nodes = [node for node in self.graph.get_all_nodes() if node.selected]
        selected_edges = [edge for edge in self.graph.get_all_edges() if edge.selected]
    ...
    def on_double_click(self, event):
        ...
        if node:
            ...
            print(f"DEBUG: Created new node {node.id} at position {world_pos} via double-click")
            self.graph_modified.emit()
            self.Refresh()
    ...
    def handle_select_down(self, pos):
        ...
        arrow_control_edge = self.get_arrow_position_control_at_position(pos.x, pos.y)
        if arrow_control_edge:
            print(f"DEBUG: 🟡 Starting to drag arrow position control of edge {arrow_control_edge.id}")
        ...
        if control_info is not None:  # Explicitly check for None
            try:
                ...
                if edge is not None and control_index is not None:  # Make sure we have valid info
                    print(f"DEBUG: 🎛️ Starting to drag control point {control_index} of edge {edge.id}")
        ...
        if endpoint_info:
            ...
            print(f"DEBUG: Starting to drag {endpoint} endpoint of edge {edge.id}")
        ...
        if node:
            ...
            if edge.is_hyperedge:
                for other_edge in self.graph.get_all_edges():
                    if other_edge.is_hyperedge and edge.shares_nodes_with(other_edge):
    ...
    def handle_select_up(self, pos):
        ...
        if self.selection_rect and self.selection_start:
            ...
            if self.selection_rect.width > 5 or self.selection_rect.height > 5:
                print(f"DEBUG: Rectangle selection with size {self.selection_rect.width}x{self.selection_rect.height}")
                # Select nodes and edges within rectangle
                self.select_items_in_rect(self.selection_rect)
            else:
                print(f"DEBUG: Single click detected (rectangle too small: {self.selection_rect.width}x{self.selection_rect.height})")
        ...
        if self.dragging_edge_endpoint and self.dragging_edge:
            ...
            if target_node:
                print(f"DEBUG: Reconnecting {self.dragging_endpoint} of edge {self.dragging_edge.id} to node {target_node.id}")
                ...
                if self.dragging_endpoint in ['from', 'to']:
                    ...
                    self.graph.modified = True
                    self.graph_modified.emit()
                elif self.dragging_endpoint.startswith('source_') or self.dragging_endpoint.startswith('target_'):
                        ...
                        if self.dragging_endpoint.startswith('source_'):
                            ...
                            if index == 0:
                                ...
                                if old_source_id in self.dragging_edge.source_ids:
                                    self.dragging_edge.source_ids[self.dragging_edge.source_ids.index(old_source_id)] = target_node.id
                            elif index - 1 < len(self.dragging_edge.source_ids):
                                # Additional source node
                                self.dragging_edge.source_ids[index - 1] = target_node.id
                        else:  # target_N
                            # Update target node
                            if index == 0:
                                # Main target node
                                ...
                                if old_target_id in self.dragging_edge.target_ids:
                                    self.dragging_edge.target_ids[self.dragging_edge.target_ids.index(old_target_id)] = target_node.id
                            elif index - 1 < len(self.dragging_edge.target_ids):
                                # Additional target node
                                self.dragging_edge.target_ids[index - 1] = target_node.id
                        
                        self.graph.modified = True
                        self.graph_modified.emit()
                    except (ValueError, IndexError):
                        print(f"DEBUG: Invalid hyperedge endpoint index: {self.dragging_endpoint}")
                else:
                ...
                if self.dragging_endpoint == 'source':
                    ...
                    print(f"DEBUG: Changed source from {old_source_id} to {new_source_id}")
                else:  # target
                    new_source_id = old_source_id
                    new_target_id = target_node.id
                    print(f"DEBUG: Changed target from {old_target_id} to {new_target_id}")
                
                # Use undo system for edge reconnection
                if self.undo_redo_manager and (new_source_id != old_source_id or new_target_id != old_target_id):
                    command = m_commands.ChangeEdgeConnectionCommand(
                        self.graph, 
                        self.dragging_edge.id,
                        old_source_id, old_target_id,
                ...
                print("DEBUG: No node found at drop position, edge reconnection cancelled")
            ...
            if self.undo_redo_manager:
                ...
                for node_id, original_pos in self.drag_original_positions.items():
                    ...
                    if node:
                        ...
                        if original_pos != current_pos:  # Only create command if position actually changed
                            command = m_commands.MoveNodeCommand(self.graph, node_id, original_pos, current_pos)
                ...
                if commands:
                    if len(commands) == 1:
                        ...
                        composite_command = m_commands.CompositeCommand("Move Nodes", commands)
                        self.undo_redo_manager.execute_command(composite_command)
    ...
    def handle_select_motion(self, pos):
        ...
        if self.dragging_edge_endpoint and self.dragging_edge:
            if self.dragging_edge.is_hyperedge and self.dragging_edge.hyperedge_visualization == "ubergraph":
                ...
                source_adjusted = self.calculate_line_endpoint(source_node, target_node,
                                                            source_screen, target_screen, True, self.dragging_edge)
                target_adjusted = self.calculate_line_endpoint(target_node, source_node,
                                                            target_screen, source_screen, False, self.dragging_edge)
                ...
                if length_sq > 0:
                    ...
                    t = ((pos.x - source_adjusted[0]) * dx + (pos.y - source_adjusted[1]) * dy) / length_sq
            ...
            # Update arrow position along the curve with edge scrolling and cursor mirroring
            if self.dragging_arrow_position_edge:
                ...
                if source_node and target_node:
                    ...
                    if edge.is_hyperedge:
                        ...
                        # Project mouse position onto the main segment
                        effective_pos = self.handle_edge_scrolling_and_cursor_mirror(pos, is_world_drag=False)
                        segment_dx = to_x - from_x
                        segment_dy = to_y - from_y
                        ...
                        if segment_length > 0:
                            ...
                            t = ((effective_pos.x - from_x) * segment_dx + (effective_pos.y - from_y) * segment_dy) / (segment_length * segment_length)
                            t = max(0.0, min(1.0, t))  # Clamp to [0, 1]
                            new_t = t
                    else:
                        # Regular edge - project onto full edge
                        effective_pos = self.handle_edge_scrolling_and_cursor_mirror(pos, is_world_drag=False)
                        dx = target_adjusted[0] - source_adjusted[0]
                        dy = target_adjusted[1] - source_adjusted[1]
                        ...
                        if edge_length > 0:
                            t = ((effective_pos.x - source_adjusted[0]) * dx + (effective_pos.y - source_adjusted[1]) * dy) / (edge_length * edge_length)
                            t = max(0.0, min(1.0, t))  # Clamp to [0, 1]
                            new_t = t
                    
                    if new_t is not None:
                        old_t = edge.arrow_position
                        edge.arrow_position = max(0.1, min(0.9, new_t))  # Keep it within reasonable bounds
                        print(f"DEBUG: 🟡 Moving arrow position from {old_t:.2f} to {edge.arrow_position:.2f}")
                        
                        # Update the main window's arrow position controls
                        main_window = self.GetParent()
                        if hasattr(main_window, 'arrow_position_slider') and hasattr(main_window, 'arrow_position_text'):
                            wx.CallAfter(main_window.arrow_position_slider.SetValue, int(edge.arrow_position * 100))
                            wx.CallAfter(main_window.arrow_position_text.SetValue, f"{edge.arrow_position:.2f}")
        ...
        if self.dragging_control_point:
            ...
            if edge:
                ...
                print(f"DEBUG: 🎮 Moving control point: screen ({pos.x}, {pos.y}) -> effective ({effective_pos.x}, {effective_pos.y}) -> world {world_pos} (rotation: {self.world_rotation}°)")
                rendering_type = edge.rendering_type if edge.rendering_type else self.edge_rendering_type
                
                if rendering_type == "composite" and getattr(edge, 'is_composite', False):
                    # Handle composite curve control points
                    segment_index = getattr(self, 'current_composite_segment', 0)
                    if (hasattr(edge, 'curve_segments') and 
                        segment_index < len(edge.curve_segments) and
                        0 <= index < len(edge.curve_segments[segment_index].get("control_points", []))):
                        
                        segment = edge.curve_segments[segment_index]
                        old_point = segment["control_points"][index]
                        segment["control_points"][index] = (world_pos[0], world_pos[1])
                        print(f"DEBUG: 🔗 Moving composite segment {segment_index} control point {index} from {old_point} to {world_pos}")
                        print(f"DEBUG: 🔗 Segment now has control points: {segment['control_points']}")
                    ...
                    if (hasattr(edge, 'control_points') and 
                        0 <= index < len(edge.control_points)):
                        old_point = edge.control_points[index]
                        edge.control_points[index] = (world_pos[0], world_pos[1])
                        edge.custom_position = True  # Mark as having custom control points
                        print(f"DEBUG: 🎛️ Moving {rendering_type} control point {index} from {old_point} to {world_pos}")
                        print(f"DEBUG: 🎛️ Edge {edge.id} control_points now: {edge.control_points}")
        ...
        if self.dragging_edge_endpoint:
            ...
            if self.dragging_edge and self.dragging_endpoint:
                ...
                if self.dragging_endpoint == 'source':
                    target_node = self.graph.get_node(self.dragging_edge.source_id)
                else:
                    target_node = self.graph.get_node(self.dragging_edge.target_id)
                
                if target_node:
                    # Calculate new anchor position based on mode
                    new_anchor = self.calculate_anchor_position(target_node, world_pos, self.edge_anchor_mode)
                    ...
                    self.dragging_edge.custom_endpoints[self.dragging_endpoint] = new_anchor
                    print(f"DEBUG: 🔗 Updated {self.dragging_endpoint} endpoint to {new_anchor} (mode: {self.edge_anchor_mode})")
            ...
            if self.drag_start_pos:
                effective_pos = self.handle_edge_scrolling_and_cursor_mirror(pos, is_world_drag=True)
    ...
    def handle_move_motion(self, pos):
        ...
        print(f"DEBUG: 🌐 Move world - Raw mouse delta: dx={dx:.1f}, dy={dy:.1f}")
        
        # Apply sensitivity scaling
        dx *= self.move_sensitivity_x
        dy *= self.move_sensitivity_y
        
        print(f"DEBUG: 🌐 Move world - After sensitivity: dx={dx:.1f}, dy={dy:.1f}")
        ...
        print(f"DEBUG: 🌐 Move world - New pan position: ({self.pan_x:.1f}, {self.pan_y:.1f})")
    ...
    def handle_rotate_down(self, pos):
        ...
        self.rotation_start_angle = math.atan2(pos.y - center_y, pos.x - center_x)
        
        print(f"DEBUG: Starting rotation from {self.initial_world_rotation:.1f}°")
        print(f"DEBUG: Click at ({pos.x}, {pos.y}), Screen center at ({center_x}, {center_y})")
        print(f"DEBUG: Initial angle: {math.degrees(self.rotation_start_angle):.1f}°")
    ...
    def handle_rotate_motion(self, pos):
        ...
        if hasattr(self, 'dragging_rotation') and self.dragging_rotation and hasattr(self, 'rotation_start_angle'):
            ...
            new_rotation = (self.initial_world_rotation + angle_delta_degrees) % 360
            if new_rotation < 0:
                new_rotation += 360
                
            print(f"DEBUG: 🔄 Mouse at ({pos.x}, {pos.y}), Center at ({center_x}, {center_y})")
            print(f"DEBUG: 🔄 Current angle: {math.degrees(current_angle):.1f}°, Start angle: {math.degrees(self.rotation_start_angle):.1f}°")
            print(f"DEBUG: 🔄 Angle delta: {angle_delta_degrees:.1f}° → New rotation: {new_rotation:.1f}°")
    ...
    def handle_rotate_element_down(self, pos):
        ...
        if node:
            print(f"DEBUG: 🎯 Selected node {node.id} for rotation at ({world_pos[0]:.1f}, {world_pos[1]:.1f})")
            ...
            self.element_rotation_start_angle = math.atan2(start_mouse_world[1] - pivot_wy,
                                                           start_mouse_world[0] - pivot_wx)
            self.show_element_rotation_center = True
            print(f"DEBUG: 🎯 Element rotation pivot (world): {self.element_pivot_world}, start_angle: {math.degrees(self.element_rotation_start_angle):.1f}°")
        else:
    ...
    def handle_rotate_element_up(self, pos):
        ...
        if self.dragging_element_rotation and self.rotating_element:
            print(f"DEBUG: 🔄 Ending element rotation - final rotation: {self.rotating_element.rotation:.1f}°")
            
            # Update edge endpoints for the rotated node
            rotated_node = self.rotating_element
            print(f"DEBUG: 🔄 Element rotation completed for node {rotated_node.id}")
            ...
            for edge in self.graph.get_all_edges():
                ...
                if needs_update:
                    print(f"DEBUG: 🔄 Updating edge {edge.id} endpoints after node rotation")
                    self.update_edge_endpoints_for_rotated_node(edge, rotated_node)
    ...
    def handle_rotate_element_motion(self, pos):
        ...
        print(f"DEBUG: 🔄🔄 Element rotation motion check - dragging: {self.dragging_element_rotation}, element: {self.rotating_element is not None}, has_angle: {hasattr(self, 'element_rotation_start_angle')}")
        # Lazy-init: if mouse is down and we have no rotating element, try to start on the fly
        try:
            if (not self.dragging_element_rotation or not self.rotating_element):
                mouse_state = wx.GetMouseState()
                if mouse_state.LeftIsDown():
                    print(f"DEBUG: 🔄 Lazy-init rotate element on motion at {pos.x},{pos.y}")
                    self.handle_rotate_element_down(pos)
        except Exception as e:
            print(f"DEBUG: ❌ Lazy-init rotate element failed: {e}")
        if self.dragging_element_rotation and self.rotating_element and hasattr(self, 'element_rotation_start_angle'):
            # Compute current mouse angle around pivot in world space
            pivot = self.element_pivot_world if self.element_pivot_world else (self.rotating_element.x, self.rotating_element.y)
            pivot_wx, pivot_wy = pivot
            mouse_world = self.screen_to_world(pos.x, pos.y)
            current_angle = math.atan2(mouse_world[1] - pivot_wy, mouse_world[0] - pivot_wx)

            # Angle delta and new rotation
            angle_delta = current_angle - self.element_rotation_start_angle
            angle_delta_degrees = math.degrees(angle_delta)
            new_rotation = (self.element_initial_rotation + angle_delta_degrees) % 360
            if new_rotation < 0:
                new_rotation += 360
            ...
            print(f"DEBUG: 🔄 Element pivot rotate: Δ={angle_delta_degrees:.1f}°, new rot={new_rotation:.1f}°, new center=({self.rotating_element.x:.1f},{self.rotating_element.y:.1f})")
            self.Refresh()
        else:
            print(f"DEBUG: ❌ Not dragging element rotation - conditions not met")
    ...
    def handle_drag_into_down(self, pos):
        ...
        print(f"DEBUG: 📦 Found {len(selected_nodes)} selected nodes and {len(selected_edges)} selected edges")
        ...
        if selected_nodes or selected_edges:
            ...
            self.dragging_selection = list(selected_nodes) + list(selected_edges)
            print(f"DEBUG: 📦 Starting drag of {len(selected_nodes)} nodes and {len(selected_edges)} edges")
            print(f"DEBUG: 📦 Selected node names: {[node.text for node in selected_nodes]}")
    ...
    def handle_drag_into_motion(self, pos):
        ...
        if self.dragging_into_container:
            ...
            target_node = self.get_node_at_position(pos.x, pos.y)
            print(f"DEBUG: 📦 Motion over target: {target_node.text if target_node else 'None'}")
            
            # Don't allow dragging into selected nodes (can't contain yourself)
            nodes_being_dragged = [item for item in self.dragging_selection if hasattr(item, 'text')]
            if target_node and target_node not in nodes_being_dragged:
                ...
                if target_node and target_node in nodes_being_dragged:
                    print(f"DEBUG: 📦 Invalid motion target - node is being dragged")
                self.container_target = None
    ...
    def handle_drag_into_up(self, pos):
        ...
        print(f"DEBUG: 📦 dragging_into_container: {self.dragging_into_container}")
        print(f"DEBUG: 📦 dragging_selection: {len(self.dragging_selection)} items")
        print(f"DEBUG: 📦 container_target: {self.container_target.text if self.container_target else 'None'}")
        
        if self.dragging_into_container:
            # Find the target node under the mouse at release point
            target_node = self.get_node_at_position(pos.x, pos.y)
            print(f"DEBUG: 📦 Target node found at release: {target_node.text if target_node else 'None'}")
            ...
            for node in self.graph.get_all_nodes():
                distance = ((node.x - world_pos[0])**2 + (node.y - world_pos[1])**2)**0.5
                print(f"DEBUG: 📦 Node '{node.text}' at ({node.x:.1f}, {node.y:.1f}), distance: {distance:.1f}, visible: {node.visible}")
            
            # Check if target is valid (not one of the selected nodes being dragged)
            if target_node:
                nodes_being_dragged = [item for item in self.dragging_selection if hasattr(item, 'text')]
                print(f"DEBUG: 📦 Nodes being dragged: {[n.text for n in nodes_being_dragged]}")
                if target_node not in nodes_being_dragged:
                    self.container_target = target_node
                    print(f"DEBUG: 📦 ✅ Valid container target: {target_node.text}")
                else:
                    print(f"DEBUG: 📦 ❌ Invalid target - cannot contain selected node in itself")
                    self.container_target = None
            else:
                print(f"DEBUG: 📦 ❌ No target node found under mouse")
                self.container_target = None
        
        print(f"DEBUG: 📦 Final check - dragging: {self.dragging_into_container}, target: {self.container_target.text if self.container_target else 'None'}")
        ...
        if self.dragging_into_container and self.container_target:
            ...
            nodes_to_contain = [item for item in self.dragging_selection if hasattr(item, 'text')]
            edges_to_contain = [item for item in self.dragging_selection if hasattr(item, 'source_id')]
            
            print(f"DEBUG: 📦 🎯 PERFORMING CONTAINMENT!")
            print(f"DEBUG: 📦 Containing {len(nodes_to_contain)} nodes and {len(edges_to_contain)} edges in {container_node.text}")
            ...
            print(f"DEBUG: 📦 Node IDs being contained: {node_ids_being_contained}")
            for edge in edges_to_contain:
                ...
                if source_contained and target_contained:
                    ...
                    print(f"DEBUG: 📦 Edge {edge.id} is internal ({edge.source_id} -> {edge.target_id}), adding to contained_edges")
                    container_node.add_contained_edge(edge.id)
                else:
                    # At least one endpoint is external - leave as external edge for redirection
                    print(f"DEBUG: 📦 Edge {edge.id} is external ({edge.source_id} -> {edge.target_id}), leaving for redirection")
                    print(f"DEBUG: 📦   Source contained: {source_contained}, Target contained: {target_contained}")
                
            # Hide contained items if container is collapsed
            if not container_node.is_expanded:
                for node in nodes_to_contain:
                    print(f"DEBUG: 📦 Hiding node '{node.text}' due to containment collapse")
                    node.visible = False
                for edge in edges_to_contain:
                    edge.visible = False
            ...
            import wx
            wx.MessageBox(f"Successfully contained {len(nodes_to_contain)} nodes and {len(edges_to_contain)} edges in '{container_node.text}'", 
            ...
            for edge_id, original_node_id in redirected_edges.items():
                ...
                if edge:
                    ...
                    if edge.source_id == target_node.id:
                        # Restore source
                        print(f"DEBUG: 📖 Restoring edge {edge_id} source from {target_node.text} to {original_node_id}")
                        edge.source_id = original_node_id
                        edges_restored += 1
                    elif edge.target_id == target_node.id:
                        # Restore target
                        print(f"DEBUG: 📖 Restoring edge {edge_id} target from {target_node.text} to {original_node_id}")
    ...
    def handle_collapse_down(self, pos):
        ...
        if target_node and target_node.is_container:
            ...
            print(f"DEBUG: 📕 Redirecting edges from internal nodes to container")
            edges_redirected = 0
            ...
            for edge in self.graph.get_all_edges():
                ...
                if source_node and source_node.id in target_node.child_ids:
                    ...
                    print(f"DEBUG: 📕 Redirecting edge {edge.id} source from {source_node.text} to {target_node.text}")
                    ...
                    print(f"DEBUG: 📕 Redirecting edge {edge.id} target from {target_edge_node.text} to {target_node.text}")
    ...
    def handle_recursive_expand_down(self, pos):
        ...
        if target_node and target_node.is_container:
            print(f"DEBUG: 🔄📖 Starting recursive expand from container {target_node.text}")
            ...
            import wx
            wx.MessageBox(f"Recursively expanded {expanded_count} containers starting from '{target_node.text}'", 
    ...
    def handle_recursive_collapse_down(self, pos):
        ...
        if target_node and target_node.is_container:
            print(f"DEBUG: 🔄📕 Starting recursive collapse from container {target_node.text}")
            ...
            # Show feedback message
            import wx
            wx.MessageBox(f"Recursively collapsed {collapsed_count} containers starting from '{target_node.text}'", 
    ...
    def recursive_expand_node(self, container_node):
        ...
        if not container_node.is_expanded:
            ...
            # Restore redirected edges to their original nodes
            print(f"DEBUG: 🔄📖 Restoring redirected edges from container {container_node.text}")
            ...
            for edge_id, original_node_id in redirected_edges.items():
                ...
                if edge:
                    ...
                    if edge.source_id == container_node.id:
                        # Restore source
                        print(f"DEBUG: 🔄📖 Restoring edge {edge_id} source from {container_node.text} to {original_node_id}")
                        edge.source_id = original_node_id
                    elif edge.target_id == container_node.id:
                        # Restore target
                        print(f"DEBUG: 🔄📖 Restoring edge {edge_id} target from {container_node.text} to {original_node_id}")
                        edge.target_id = original_node_id
    ...
    def recursive_collapse_node(self, container_node):
        ...
        if container_node.is_expanded:
            ...
            # Redirect edges from internal nodes to the container
            print(f"DEBUG: 🔄📕 Redirecting edges from internal nodes to container {container_node.text}")
            edges_redirected = 0
            ...
            for edge in self.graph.get_all_edges():
                ...
                if source_node and source_node.id in container_node.child_ids:
                    # Source is internal - redirect source to container
                    print(f"DEBUG: 🔄📕 Redirecting edge {edge.id} source from {source_node.text} to {container_node.text}")
                    ...
                    # Target is internal - redirect target to container
                    print(f"DEBUG: 🔄📕 Redirecting edge {edge.id} target from {target_edge_node.text} to {container_node.text}")
                    container_node.add_redirected_edge(edge.id, edge.target_id)
                    edge.target_id = container_node.id
                    edges_redirected += 1
            
            print(f"DEBUG: 🔄📕 Redirected {edges_redirected} edges to container {container_node.text}")
            collapsed_count += 1
    ...
    def draw_container_target_feedback(self, dc):
        ...
        screen_pos = self.world_to_screen(self.container_target.x, self.container_target.y)
    ...
    def draw_drag_into_feedback(self, dc):
        ...
        dc.SetBrush(wx.Brush(wx.Colour(255, 165, 0, 80)))  # Semi-transparent orange
        ...
        for item in self.dragging_selection:
            if hasattr(item, 'text'):  # It's a node
                ...
                drag_rect = wx.Rect(
                    int(visual_x - width // 2),
                    int(visual_y - height // 2),
                    width,
                    height
                )
        ...
        if self.edge_start_node:
            ...
            if target_node:  # Allow self-loops by removing the != check
                ...
                if self.edge_start_connection:
                    ...
                    if not self.check_edge_restrictions(self.edge_start_node, target_node):
                        self.temp_edge = None
                        self.temp_path_points = []
                        return
                ...
                # Create the edge
                edge = m_edge.Edge(source_id=self.edge_start_node.id, target_id=target_node.id)
                if self.edge_rendering_type == "freeform" and self.temp_edge:
                    # Use the collected path points for the new edge
                    edge.rendering_type = "freeform"
                    edge.freeform_points = self.temp_path_points.copy()  # Make a copy to avoid reference issues
                ...
                try:
                    ...
                    if hasattr(main_window, 'edge_directed_cb'):
                        ...
                        print(f"DEBUG: Created edge with directed={directed_value} from UI checkbox")
                    else:
                        print("DEBUG: Could not find edge_directed_cb, using default directed=True")
                        edge.directed = True
                except Exception as e:
                    print(f"DEBUG: Error accessing MainWindow, using default directed=True: {e}")
                    edge.directed = True
    ...
    def check_edge_restrictions(self, source_node, target_node):
        ...
        if self.no_loops and source_node.id == target_node.id:
            wx.MessageBox("Cannot create self-loop: No loops restriction is enabled.", 
                         "Graph Restriction", wx.OK | wx.ICON_WARNING)
            print(f"DEBUG: 🔒 Blocked self-loop creation due to no_loops restriction")
            return False
        
        # Check for multigraph restriction
        if self.no_multigraphs:
            # Check if an edge already exists between these nodes (in either direction)
            existing_edges = self.graph.get_all_edges()
            for edge in existing_edges:
                if ((edge.source_id == source_node.id and edge.target_id == target_node.id) or
                    (edge.source_id == target_node.id and edge.target_id == source_node.id)):
                    wx.MessageBox(f"Cannot create multiple edges between the same nodes: No multigraphs restriction is enabled.", 
                                 "Graph Restriction", wx.OK | wx.ICON_WARNING)
                    print(f"DEBUG: 🔒 Blocked multiple edge creation due to no_multigraphs restriction")
                    return False
            ...
            try:
                ...
                if hasattr(main_window, 'edge_directed_cb'):
                    ...
                    if self.graph_type_restriction == 1:  # Require Directed Graph
                        if not is_directed:
                            wx.MessageBox("Cannot create undirected edge: Graph requires all edges to be directed.", 
                                         "Graph Restriction", wx.OK | wx.ICON_WARNING)
                            print(f"DEBUG: 🔒 Blocked undirected edge creation due to directed-only restriction")
                            return False
                    
                    elif self.graph_type_restriction == 2:  # Require Undirected Graph  
                        if is_directed:
                            wx.MessageBox("Cannot create directed edge: Graph requires all edges to be undirected.", 
                                         "Graph Restriction", wx.OK | wx.ICON_WARNING)
                            print(f"DEBUG: 🔒 Blocked directed edge creation due to undirected-only restriction")
                            return False
    ...
    def select_items_in_rect(self, rect):
        ...
        print(f"DEBUG: Selecting items in screen rect: {rect.x}, {rect.y}, {rect.width}, {rect.height}")
        ...
        world_poly = [self.screen_to_world(int(px), int(py)) for (px, py) in screen_corners]
        ...
        def point_in_polygon(point, polygon):
            ...
            for i in range(n):
                x1, y1 = polygon[i]
                x2, y2 = polygon[(i + 1) % n]
                # Check if point is between y1 and y2 (inclusive on lower bound)
                if ((y1 > y) != (y2 > y)):
        ...
        for node in self.graph.get_all_nodes():
            ...
            node_poly = self._get_node_corners(node)  # world corners with node rotation

            if polygons_intersect(node_poly, world_poly) or point_in_polygon((node.x, node.y), world_poly):
                self.graph.select_node(node.id)
                nodes_selected += 1
                print(f"DEBUG: ✅ Selected node '{node.text}' via world polygon intersection")
            else:
                print(f"DEBUG: ❌ Node '{node.text}' not intersecting world selection polygon")
        ...
        print(f"DEBUG: World polygon selection found {nodes_selected} nodes and {edges_selected} edges")
    ...
    def edge_intersects_world_polygon(self, edge, world_polygon):
        ...
        for i in range(samples + 1):
            t = i / samples
            curve_world_point = self.calculate_position_on_curve(edge, source_node, target_node, t)
            ...
            if curve_world_point:
                ...
                if prev_world_point:
                    for j in range(len(world_polygon)):
                        ...
                        if self.line_segments_intersect(prev_world_point, curve_world_point, q1, q2):
                            return True
    ...
    def line_segment_intersects_rect(self, p1, p2, rect):
        ...
        for rect_edge in rect_edges:
            if self.line_segments_intersect(p1, p2, rect_edge[0], rect_edge[1]):
                return True
    ...
    def update_edge_endpoints_for_rotated_node(self, edge, rotated_node):
        ...
        if edge.source_id == rotated_node.id:
            ...
            if hasattr(edge, 'custom_endpoints') and 'source' in edge.custom_endpoints:
                ...
                print(f"DEBUG: 🔄 Would update custom source endpoint: {old_endpoint}")
            else:
                # For automatic endpoints, select the best anchor point based on edge direction
                world_target_pos = (target_node.x, target_node.y)
                best_anchor = self.calculate_anchor_position(rotated_node, world_target_pos, "nearest_face")
        ...
        if edge.target_id == rotated_node.id:
            ...
            if hasattr(edge, 'custom_endpoints') and 'target' in edge.custom_endpoints:
                # For custom endpoints, recalculate the position based on the node's rotation
                old_endpoint = edge.custom_endpoints['target']
                # TODO: Apply rotation transformation to the custom endpoint
                print(f"DEBUG: 🔄 Would update custom target endpoint: {old_endpoint}")
            else:
                # For automatic endpoints, select the best anchor point based on edge direction
                world_source_pos = (source_node.x, source_node.y)
                best_anchor = self.calculate_anchor_position(rotated_node, world_source_pos, "nearest_face")
                if not hasattr(edge, 'custom_endpoints'):
                    edge.custom_endpoints = {}
